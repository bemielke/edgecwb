 ...
 * The FP6 algorithm processes a set of bands with center periods Tn and bandwidth Wb given by the ratio Tn/Tn-1. For each filter band the data
 ...
 the maximum CF within tUp and tUpMin, and uses the threshold values threshold1 and thresh2.
 <p>
 ...
 * <p>
 * Declared picks are stored in an ArrayList of FilterPicker6PickData objects, public ArrayList FilterPicker6.getPickData() should be called after
 * each call to FilterPicker6.apply() to recover new picks.
 * <p>
 * FilterPicker6 uses an automatically created memory object FilterPicker6_Memory to allow support re-entry to picker for contiguous data samples from
 * the same channel. Consequently, an instance of FilterPicker6 can only be used for contiguous data from a single channel. If there is a gap in the
 * data, FilterPicker6.initializeMemory() should be invoked before the next call to FilterPicker6.apply()
 * <p>
 * Pseudocode example of how FilterPicker6 can be used:
 * <p>
 * <blockquote><pre>
 // Create a FilterPicker6 object:
 // initialize: String localeText, double longTermWindowFactor,
 //     double threshold1, double thresh2,
 //     double tUp, double tUpMin
 ...
 // EITHER:
 // construct a FilterPicker6 object  with a filtWindow ...
 // initialize: double filtWindow
 ...
 FilterPicker6 filterPicker6 = new FilterPicker6(locale, longTermWindowFactor, threshold1, thresh2, tUp, tUpMin, filtWindow);
 // OR:
 // construct a FilterPicker6 object  with a filtWindow
 // initialize: FilterPicker6_BandParameters[] bandParameters
 ...
 FilterPicker6 filterPicker6 = new FilterPicker6(locale, bandParameters, longTermWindowFactor, threshold1, thresh2, tUp, tUpMin, filtWindow);
 ...
 * </pre></blockquote>
 * <p>
 * For examples of how FilterPicker6 can be used through calls to its get and set methods and in an interactive environment, see
 ...
 * @author anthony
 */
public class FilterPicker6 extends BasicPicker {
     ...
     * method. Note that the ratio of each band period band to previous shorter band period must be approximately constant, see bwFactor.
     */
    protected FilterPicker6_BandParameters[] bandParameters = new FilterPicker6_BandParameters[0];
    /**
     * // _DOC_ Filter window (filtWindow) in seconds sets the approximate longest filter band period when the bands are not explicitly set. The
 ...
 are created, starting with start from bwFactor * Nyquist period (2*deltaTime).
     */
 ...
 tUpMin.
    ...
    /**
     * // _DOC_ thresh2 sets the threshold to declare a pick event (pick will be accepted when tUp reached). This threshold is reached when
 the integral of the maximum (clipped) characteristic function over all filter bands within the window tUp exceeds thresh2 * tUp
 (i.e. the average (clipped) characteristic function over window tUp is greater than thresh2).
     */
    protected double threshold2 = THRESHOLD2_DEFAULT;       // threshold to maintain trigger
    /**
     * // _DOC_ tUp sets the maximum time window after threshold1 is reached (pick event triggered) in which the integral of the (clipped)
 characteristic function is accumulated to declare a pick. A pick is declared if this integral exceeds thresh2 * tUp.
     */
    protected double tUpEvent = TUPEVENT_DEFAULT;
    /**
     * // _DOC_ tUpMin sets a minimum time window after threshold1 is reached (pick event triggered) in which an integral of the (clipped)
 characteristic function is accumulated to confirm a pick. A pick is confirmed if this integral exceeds threshold1 * tUpMin.
     */
 ...
 bwFactor is set automatically if band parameters are initialized explicitly.
     */
    ...
    * @throws net.alomax.timedom.TimeDomainException
     */
    public FilterPicker6(String localeText, FilterPicker6_BandParameters[] bandParameters, double longTermWindowFactor, double threshold1, double threshold2,
            double tUpEvent, double tUpEventMin) throws TimeDomainException {

        super(localeText);

        // new, FilterPicker6 data fields
        ...
        // cannot use both bandParameters and filtWindow parameters
        this.filterWindow = FILTER_WINDOW_NOT_USED;

    }

    /**
     * Constructs a new FilterPicker6 object setting filter bands automatically based on filtWindow and sample interval of data
     *
     * @param localeText locale for information, warning and error messages
     * @param longTermWindowFactor long term window factor
     * @param threshold1 threshold to trigger a pick event
     ...
     */
    public FilterPicker6(String localeText, double longTermWindowFactor, double threshold1, double threshold2,
        ...
        // cannot use both bandParameters and filtWindow parameters
        this.bandParameters = null;  // changing filtWindow will change picker parameters (numBands)
     ...
     * @throws net.alomax.timedom.TimeDomainException
     */
    public FilterPicker6(FilterPicker6 tp) throws TimeDomainException {
    ...
    /**
     * @return the saveBandCharFunc
     */
    public boolean isSaveBandCharFunc() {
        return saveBandCharFunc;
    }

    /**
     * @param saveBandCharFunc the saveBandCharFunc to set
     */
    public void setSaveBandCharFunc(boolean saveBandCharFunc) {
        this.saveBandCharFunc = saveBandCharFunc;
    }
    ...
    * @throws net.alomax.timedom.TimeDomainException
     ...
     * Sets bwFactor
     *
     * @param bandParameters band parameters whose period ratios will determine bwFactor
    ...
    public void setBandWidthFactor(FilterPicker6_BandParameters[] bandParameters) throws TimeDomainException {
        ...
        for (int n = 1; n < bandParameters.length; n++) {
            ...
            if (var > BANDWIDTH_THRES_ERROR) { // 10.
                bwError = true;
            }
            if (var > BANDWIDTH_THRES_WARNING) { // 2.
                bwWarning = true;
            }
        }
        ...
        if (bwWarning || bwError) {
            ...
            Logger.getLogger(FilterPicker6.class.getName()).log(Level.INFO, message);
            ...
            Logger.getLogger(FilterPicker6.class.getName()).log(Level.INFO, text);
            if (bwError) {
        ...
        //this.bwFactor = bwFactor;   // DCK: assignment to self
    }
    ...
    public void setNumPolesBandPass(String str) throws TimeDomainException {
        ...
        FilterPicker6.this.setNumPolesBandPass(numPolesLowPassValue);
    }
    ...
    public void setLongTermWindowFactor(String str) throws TimeDomainException {
        ...
        FilterPicker6.this.setLongTermWindowFactor(longTermWindowValue);
    }
    ...
    /**
     * Sets thresh2, threshold to to declare a pick
     *
     * @param threshold2 threshold to declare a pick
     * @throws TimeDomainException
     */
    ...
    // DEBUG & ANALYSIS
    private static int index_band = 0;
    public static final boolean DEBUG = false;
    public static boolean DEBUG_PICK = false;
    // working arrays
    ...
    /**
     * Flag to enable/disable storing of clipped characteristic function values for each band for all samples passed to apply().
     */
    protected boolean saveBandCharFunc = false;
    /**
     * Array to store clipped characteristic function values for each band for all samples passed to apply().
     * Values are only stored if setSaveBandCharFunc(true) is called after FilterPicker6 construction.
     * Values can be retrieved by calling public float[][] getBandCharFunc() after each call to apply().
     */
    protected float[][] bandCharFunc = null;

    public void setDebugPick(boolean t) {DEBUG_PICK=t;}
    /**
     * @return the bandCharFunc
     */
    public float[][] getBandCharFunc() {
        return bandCharFunc;
    }

    @Override
    public final float [] apply(double dt, float [] sample)  throws TimeDomainException {
      return apply(dt, sample, sample.length);
    }
    /**
     ...
     * @param npts Number of samples being presented
     * @return modified sample array if resultType == TRIGGER || resultType == CHAR_FUNC
     * @throws net.alomax.timedom.TimeDomainException
     */
    public final float[] apply(double dt, float[] sample, int npts) throws TimeDomainException {
        ...
        if (fp6Memory == null) {
            ...
            bandCharFunc = new float[numBands][];
        }
        ...
        if (saveBandCharFunc) {
            for (int nband = 0; nband < numBands; nband++) {
              if(bandCharFunc[nband] == null) {
                bandCharFunc[nband] = new float[sample.length];
              }
              else if(bandCharFunc[nband].length < sample.length ) bandCharFunc[nband] = new float[sample.length];// is it big enough
            }
        }
        ...
        int nSamplesReset = 0; // Value to check CF is below value for x samples -- Added WLY
        int indexUpEventTrigger = -1;
        int indexUncertaintyPick = -1;
        for (int nsamp = 0; nsamp < npts; nsamp++) {
            ...
            for (int nband = 0; nband < numBands; nband++) {
                /*   // 20150314 AJL
                 ...
                 // _DOC_  http://en.wikipedia.org/wiki/High-pass_filter    y[i] := α * (y[i-1] + x[i] - x[i-1])
                 int fsIndex = 0;
                 ...
                 // _DOC_  http://en.wikipedia.org/wiki/Low-pass_filter    y[i] := y[i-1] + α * (x[i] - y[i-1])
        ...
        // _DOC_  apply N-pole BP filter
                ...
                if (fp6Memory.mean_stdDev_xRec[nband] <= Float.MIN_VALUE) {
                    if (fp6Memory.enableTriggering && error1_not_printed) {
                        Logger.getLogger(FilterPicker6.class.getName()).log(Level.SEVERE, 
                    ...
                    if (saveBandCharFunc) {
                        bandCharFunc[nband][nsamp] = (float) (charFunctTest / fp6Memory.thresholdScaleFactor[nband]);
                    }
                }
            }
            for (int nband = 0; nband < numBands; nband++) {
// AJL 20091214
                ...
                boolean upCharFunctUncertainty // char funct increases past uncertaintyThreshold
                        = ((fp6Memory.charFunctUncertaintyLast[nband] < fp6Memory.uncertaintyThreshold[nband]) && (fp6Memory.charFunctUncertainty[nband] >= fp6Memory.uncertaintyThreshold[nband]));
...
// END - AJL 20091214
            ...
            if (fp6Memory.enableTriggering || fp6Memory.nTotal++ > fp6Memory.indexEnableTriggering) {  // past stabilisation time
                ...
                if (beginTUpEventPtrLast < 0) {
                    Logger.getLogger(FilterPicker6.class.getName()).log(Level.SEVERE, "FilterPicker6: beginTUpEventPtr < 0 (this should not happen!)");
                }
                ...
                if (beginTUpEventPtrShort < 0) {
                    Logger.getLogger(FilterPicker6.class.getName()).log(Level.SEVERE, "FilterPicker6: beginTUpEventPtrShort < 0 (this should not happen!)");
                }
                ...
                fp6Memory.integralCharFunctClippedShort[fp6Memory.sampleMemBufPtr]
                        = fp6Memory.integralCharFunctClippedShort[sampleMemBufPtrLast] - fp6Memory.charFunctClippedValue[beginTUpEventPtrShort] + charFunctClippedMaxOverBands;
                ...
                // _DOC_ if new picks allowed, check if integralCharFunct over last tUp window is greater than threshold
                if (fp6Memory.allowNewPickIndex != INT_UNSET && fp6Memory.integralCharFunctClipped[fp6Memory.sampleMemBufPtr] >= fp6Memory.criticalIntegralCharFunct) {

                    // _DOC_ find first point in tUp window where charFunctMaxOverBands rose past threshold1
                    ...
                    int icount = fp6Memory.nTUpEvent; // counts back from current end of tUp window
                    ...
                    if (acceptedPick) {
                        ...
                        /*
                         ...
                         // measure polarity on longest period band that triggered in tUp window for polarity
                         for (int nband = numBands - 1; nband >= 0; nband--) {
                        ...
                        if (iPick < 0) {
                            Logger.getLogger(FilterPicker6.class.getName()).log(Level.SEVERE, "FilterPicker6: iPick < 0 (this should not happen!)");
                        }
                        ...
                        for (int nband = 0; nband < numBands; nband++) {
                            ...
                            if (indexBandTrigger[nband] >= 0) {   // measure polarity on all bands that triggered in tUp window
            ...
            if (resultType == PICKS) {                // PICK
                if (acceptedPick) {
                    ...
                    FilterPicker6PickData pickData = this.getNewFilterPickData((double) indexBeginPick, (double) indexEndPick,
        ...
        if (useMemory) {
            ...
            for (int i = 0; i < fp6Memory.nSampleMemory; i++) {
                // AJL 20091214
                for (int nband = 0; nband < numBands; nband++) {
                    fp6Memory.indexUncertainty[nband][i] -= npts;
                }
                // END - AJL 20091214
            }
            if (fp6Memory.allowNewPickIndex != INT_UNSET) {
                fp6Memory.allowNewPickIndex -= npts;
            }
            fp6Memory.canAllowNewPickIndex -= npts;
        } else {
            fp6Memory = null;
        }
        if (resultType == BANDS) {
    ...
    public void setFilterWindow(String str) throws TimeDomainException {
        ...
        try {
            filtWindow = Double.valueOf(str);
        } catch (NumberFormatException e) {
            throw new TimeDomainException(TimeDomainText.invalid_filterWindow_value + ": " + str);
        }

        setFilterWindow(filtWindow);
    }

    /**
     * Initialize filter bands automatically. Bands will be spaced by bwFactor from bwFactor * deltaTime * 2.0 to first period greater
 than specified filtWindow; thresholdScaleFactor is set to 1.0
 ...
 gives backwards compatibility with FilterPicker5
    ...
    protected void initializeBandsAuto(double filterWindow) throws TimeDomainException {
        ...
        //double period0 = bwFactor * deltaTime;
        // 20150522 AJL - start from bwFactor * Nyquist period (2*dt)
...
// _DOC_ =============================
    public class FilterPicker6_Memory extends TimeDomainMemory {
        ...
        // initialize bands and filter parameters if number of bands and band periods have not been specified
        {
            if (bandParameters == null || bandParameters.length < 1) {
                try {
                    ...
                    Logger.getLogger(FilterPicker6.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        ...
        /* 20150214 AJL - FP5->FP6
         ...
         {
         int nTemp = 1;
         int numPrevious = (int) (filtWindow / deltaTime);  // estimate of number of previous samples to bufer
...

        {
            ...
            for (int nband = 0; nband < numBands; nband++) {
                ...
                //double fmultiplier = bwFactor;
                double fmultiplier = Math.pow(bandWidthFactor, 1.25);    // 1.5 gives best reconstruction of delta function DEBUG - try narrower
        ...
        /*
         ...
         double bandwidth = deltaTime * 2.0 * (bwFactor * freq - freq);
        ...
        //double maxCharFunctValue = 5.0 * thresh2; // 20150318 AJL - changed to thresh2 since threshold1 could be set very low.
        ...
        // _DOC_ criticalIntegralCharFunct is tUp * thresh2
        ...
        // flag to prevent next trigger until charFunc drops below thresh2
        ...
        /** construct the memory buffers/variables
         *
         */
      ...
      /** these FilterPickData array list are used to allow the creation of few of these and reuse of the objects.
   * 
   */
  private final ArrayList<FilterPicker6PickData> pickDataUsed = new ArrayList<>(5);
  private final ArrayList<FilterPicker6PickData> pickDataFree = new ArrayList<>(5);

  /** Put all picks onto the free list - essentially reset the pool to all free.
   * 
   */
  private void freePickPool() {
    if(pickDataUsed.isEmpty()) return;
    for(int i=pickDataUsed.size()-1; i>= 0; i--) {
      pickDataFree.add(pickDataUsed.get(i));
      pickDataUsed.remove(i);
    }
  }
  /** This mimics the constructor of PickData used by filter picker6.  Par
   * 
   * @param index0 Starting index of pick range
   * @param index1 Ending index of pick range
   * @param polarity The polarity of pick (POLARITY_POS, POLARITY_NEG, or POLARITY_UNKNOWN).
   * @param amplitude A value representative of the pick strength (e.g. maximum characteristic function value within pick detection window).
   * @param amplitudeUnits Amplitude units for amplitude field (NO_AMP_UNITS, CHAR_FUNCT_AMP_UNITS, etc.).
   * @param period A characteristic period for the pick.
   * @param nSamplesUpEventUsed #samps after trigger required for the integral of the (clipped) characteristic function to exceeds threshold2 * tUpEvent (e.g.
     * number of samples needed to declare this pick)
   * @param bandParameters array over bands of filter band parameters (band periods and thresholdScaleFactor) used for this pick.
   * @param charFunctValueTrigger array over bands of unclipped characteristic function values at trigger sample for this pick.
   * @param charFunctValueMaxInUpEventWindow array over bands of maximum, unclipped characteristic function values within window from trigger to in window from pick accepted to sample at
     * which pick was declared.
   * @return A object from the pool of such objects with these values set.
   */
  private FilterPicker6PickData getNewFilterPickData(double index0, double index1, int polarity, double amplitude, String amplitudeUnits, double period,
      ...
      FilterPicker6PickData pick;
      if(pickDataFree.isEmpty()) {
        pick = new FilterPicker6PickData(index0, index1, polarity, amplitude, amplitudeUnits, period,
            nSamplesUpEventUsed,
            bandParameters, charFunctValueTrigger, charFunctValueMaxInUpEventWindow);
      }
      else {
        pick = pickDataFree.get(pickDataFree.size()-1);
        pick.reload(index0, index1, polarity, amplitude, amplitudeUnits, period,
            nSamplesUpEventUsed,
            bandParameters, charFunctValueTrigger, charFunctValueMaxInUpEventWindow);
        pickDataUsed.add(pick);
        pickDataFree.remove(pickDataFree.size()-1);
      }
      return pick;
  }
}	// End class
