...
public class TimeInstant extends Date2 implements Comparable, Serializable {
    ...
    public final static double SEC_PER_YEAR = SEC_PER_DAY * 365.5;  // except leap year!
    ...
    public String toString(int format, int ndec, boolean pretty) {
        ...
        if (pretty) {
            return (new String(
                    toString2(format) + "  " + NumberFormat.intString(ihour, 2) + "h" + NumberFormat.intString(imin, 2) + "m"
                            + NumberFormat.floatString((float) sec, ndec > 0 ? ndec + 3 : 2, ndec) + "s"));
        } else {
            return (new String(
                    toString2(format) + " " + NumberFormat.intString(ihour, 2) + NumberFormat.intString(imin, 2) + " "
                            + NumberFormat.floatString((float) sec, ndec > 0 ? ndec + 3 : 2, ndec)));
        }
    ...
    public String toStringSeparated(String dateSeparator, String dateTimeSeparator, String timeSeparator, int ndec) {
        ...
        String secStr = NumberFormat.floatString((float) sec, ndec > 0 ? ndec + 3 : 2, ndec);
        //if (ndec <= 0) {
    ...
    public String toStringInvert(int format, int ndec, boolean pretty) {
        ...
        if (pretty) {
            return (new String(
                    ...
                    ndec > 0 ? ndec + 3 : 2, ndec) + "s" + "  " + toString2(format)));
        } else {
            return (new String(
                    NumberFormat.intString(ihour, 2) + NumberFormat.intString(imin, 2) + " " + NumberFormat.floatString(
                    (float) sec, ndec > 0 ? ndec + 3 : 2, ndec) + " " + toString2(format)));
        }
