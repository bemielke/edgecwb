...
import gov.usgs.anss.edgemom.ISILink;
import gov.usgs.anss.edge.RawDisk;
import java.io.IOException;
import java.io.File;

/**
 * ISIGapFiller.java - This class monitors an ISI configuration directory for
 * *.gap files and creates ISILink.java to fill in the gaps. It manages the .gap
 * file to reflect the gaps filled and deletes the .gap file if it is ever
 * completed.
 *
 *
 * @author davidketchum
 */
public final class ISIGapFiller extends Thread {

  //private TreeMap<String, ISIGapFiller> fillers= new TreeMap<>();
  ...
  private final ShutdownISIGapFiller shutdown;
   ...
   */
  public ISIGapFiller(String argline, String tg, EdgeThread parent) {
    arglin = "";
    par = parent;
    ...
    /*if(shutdown != null) {
      par.prt(tag+" ISIGap: ******** attempt to create another ISIGapFiller!");
      return;
    }*/
    par.prt(tag + "ISIGap: created args=" + argline + " dir=" + dirStub + " filename=" + filename + " arglin=" + arglin);
    shutdown = new ShutdownISIGapFiller();
  ...
  public void run() {
    ...
    try {     // this try catches any RuntimeExceptions
      ...
      try {
        ...
        par.prta(tag + " ISIGap: Cannot open " + filename + ".caughtup");
        Util.IOErrorPrint(e, tag + "ISIGap: Cannot open " + filename + ".caughtup", par.getPrintStream());
      }
      File dir = new File(dirStub);
      ISILink isi = null;
      par.prta(tag + " ISIGap: path=" + dirStub + " dir=" + dir.toString() + " isDir=" + dir.isDirectory() + " dbg=" + dbg);
      while (!terminate) {
        try {
          sleep(10000);
        } catch (InterruptedException expected) {
        ...
        if (isi != null) {
          if (!isi.isRunning()) {
            par.prta(tag + Util.ascdate() + " ISIGap: file completed on " + isi);
            isi = null;
          }
          ...
          for (String file : files) {
            if (file.contains(filename + ".gap")) {
              par.prta(Util.ascdate() + tag + " ISIGap: create Gap fill ISILink " + arglin + " -s " + dirStub + "/" + file);
              try {
                isi = new ISILink(arglin + "-rcvsiz 4096 -s " + dirStub + "/" + file + " >>" + file, file);
                par.prta(tag + "ISIGap: tag created=" + isi.getTag() + " gap size=" + isi.getGapSize());
              } catch (RuntimeException e) {
                par.prta(Util.ascdate() + tag + " ISIGap: could not start ISILink for gap file=" + file + " e=" + e);
                e.printStackTrace(par.getPrintStream());
                e.printStackTrace();
                SendEvent.debugEvent("ISIGapRunTim", tag + " Runtime=" + e + " " + file, this);
              }
              done = true;
              break;
            }
          ...
          if (!done) {
            if (loop++ % 360 == 0 && updFileWriter != null) {
              try {
                ...
                par.prta(tag + "ISIGap: Did not update " + filename + ".caughtup");
                Util.IOErrorPrint(e, tag + "ISIGap: Did not update " + filename + ".caughtup", par.getPrintStream());
              }
            }
          }
        }
      ...
      par.prta(tag + "ISIGap: RuntimeException in " + this.getClass().getName() + " e=" + e.getMessage());
    ...
    par.prta(tag + "ISIGap: ** ISIGapFiller terminated terminate=" + terminate);
   ...
   * exit
   */
  private final class ShutdownISIGapFiller extends Thread {

    /**
     * default constructor does nothing the shutdown hook starts the run()
     * thread
     */
    public ShutdownISIGapFiller() {
    ...
    @Override
    public void run() {
      System.err.println(tag + "ISIGap: ISIGapFiller Shutdown() started...");
      ...
      System.err.println(tag + "ISIGap: Shutdown() of ISIGapFiller is complete.");
    }
  }
}
