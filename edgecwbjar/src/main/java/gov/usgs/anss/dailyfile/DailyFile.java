  ...
  public boolean add(byte[] buf, int off, int len) throws IOException, IllegalSeednameException {
    if (len == 512) {
      ...
      if (!inOrder) {
        ...
        if (now - lastSort > sortTime /*&& now - lastSortAny > 30000*/) {  // is it time to sort it
  ...
  private void sortFile() throws IOException {
    synchronized (blks) {
      ...
      while (ioff < rw.length()) {
        try {
          if(sortBuf[0] == 0 && sortBuf[1] == 0 && sortBuf[8] == 0 && sortBuf[15] == 0) {
            prta("DF: sortFile() Zero block found while sorting ioff="+ioff+" "+toString());
          }
          else {
