  ...
  public static void staticprt(String s) {
    if(!useConsole) {
      ...
      if((staticout == null || staticlastday != System.currentTimeMillis()/86400000L )) {
        ...
        try {
          staticout = new PrintStream(
                  new FileOutputStream(Util.getProperty("logfilepath") == null? "":Util.getProperty("logfilepath")+
          staticlogname+".log"+EdgeThread.EdgeThreadDigit(), append));// append if out is null
        }
        catch(FileNotFoundException e) {
          System.out.println("Could not open log file to "+(Util.getProperty("logfilepath") == null? "":Util.getProperty("logfilepath"))+
          staticlogname+".log"+EdgeThread.EdgeThreadDigit()+" e="+e);
          System.exit(1);
        }
