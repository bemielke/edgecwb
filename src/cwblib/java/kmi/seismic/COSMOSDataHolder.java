...
/**
 * This is a class to hold data for a file to be stored in COSMOS format. This code is currently set
 * up for COSMOS v1.20, although it is intended to be flexible enough to grow and adapt as the
 * format does.<p>
 * The class is set up to read the contents of the various headers from an input file, along with
 * several deployment specific variables. Variables and header fields are defined within the code
 * based on index number (say of integer values) or based on line number and column position within
 * the headers.<p>
 * The class provides generic methods for setting the values without in most cases knowing the
 * details of the variable itself.<p>
 * See COSMOSTESTER.JAVA for an example.<p>
 *
 * v1.1 Add reader of KMI generated COSMOS event files readFile(filename)()
 * <p>
 *
 ...
 * @version 1.1
 */
public final class COSMOSDataHolder {
  // Class variables
...
// --------------------------------------------------------------------
  ...
  public String getComment(int idx) {
    return (String) _commentHeader.get(idx);
  }

  public void setComment(int idx, String s) {
    _commentHeader.set(idx, s);
  }

  public void addComment(int idx, String s) {
    _commentHeader.add(idx, s);
  }
  ...
  /**
   * Constructor, Creates a COSMOSDataHolder object for reading. No passed arguments, will be read
   * from file
   */
  ...
  private void defineVars() {
    ...
    defineHeaderVar("RECORDERSN", HDR_TEXT, TYPE_INTEGER, 7, 21, 6);  // DCK was 5, now 6
    ...
    defineHeaderVar("CHANNEL", HDR_COMMENT, TYPE_STRING, 1, 13, 3);
    defineHeaderVar("CHANNEL", HDR_COMMENT, TYPE_STRING, 1, 13, 3);
   ...
   * These are doing the first basic checks of style and setting the most basic defaults such as the
   * values for unknown.<p>
   * Some of these values may be overridden by .set commands in the style file.
   *
   * @return true if initialzed properly, false if not.
   */
  private boolean presetVars() {
    ...
    if ((_commentHeader.size() < 2) || (_commentHeader.size() > 500)) {
   ...
   * For the most part, this just allocates and loads the integer and real arrays with the defaults
   * that may or may not have been changed by .set commands in the style file.<p>
  ...
  /**
   * Make a filename from components. Note that you must limit length of string components yourself
   * if you care.
   *
...
//              UserMsg.debug("idx= "+idx+" start= "+start+" end= "+end+"  ii= "+ii);
   ...
   * specific variables from the file. Keywords are denoted by a leading "." and include
   * ".textheader", ".integerheader", ".realheader", ".commentheader", ".dataheader", ".trailer",
   * and ".set"
   * .<p>
   * Column alignment helpers "12345..." are allowed and will be ignored, as are comment lines that
   * begin with ";".
   *
   * @param style - Style string
   * @return true if success, false if not.
   */
  public boolean loadStyleString(String style) {
    try {
      BufferedWriter file = new BufferedWriter(new FileWriter("cosmos$$$.cfg"));
      file.write(style);
      file.close();
    } catch (IOException e) {
      // Do nothing
    ...
    boolean ret = loadStyle("cosmos$$$.cfg");
    File f = new File("cosmos$$$.cfg");
    if (f.exists()) {
      f.delete();
    }
    return ret;
  ...
  public boolean loadStyle(String stylefile) {
    ...
    try {
      ...
      BufferedReader fil = new BufferedReader(new StringReader(stylefile));
  ...
  /**
   * Set scaling information. This is done very late in the game, just before writing the file out.
   * This to make sure that user preferences are all honored.
   */
  ...
  /**
   * Set units. Based on unit code, set processing stage, data type (in multiple fields), unit
   * descriptor and data format.
   */
  ...
  /**
   * Header variable class holds variable name, type, header (or not), and position in data tables
   * or header.
   */
  private class headerVar {
    ...
    public String toString() {
      return _varname + " hdr=" + _varhdr + " _typ=" + _vartyp + " hdrline=" + _hdrline + " hdridx=" + _hdrix + " len=" + _varlen;
    }
        ...
        if (hval._varname.equals(varname)) {
          ...
          if (hval._vartyp == TYPE_INTEGER) {
            ...
            if (wstr.length() > hval._varlen) {
              wstr = wstr.substring(wstr.length() - hval._varlen); //DCK - limit int to width of field take right most
    ...
    public void setString(String varname, String val) {
      ...
      for (int lix = 0; lix < lsize; lix++) {
        headerVar hval = (headerVar) _hdrVars.get(lix);
        if (varname.equals("RECORDERSN")) {
          System.out.println("got recordersn " + val + " len=" + hval._varlen);
        }
        if (hval._varname.equals(varname)) {
          ArrayList hdr = getHeaderByType(hval._varhdr);
          String str = (String) hdr.get(hval._hdrline);
