...
/**
 * RequestType.java If the name of the class does not match the name of the field in the class you
 * may need to modify the rs.getSTring in the constructor.
 *
 * This RequestType templace for creating a database database object. It is not really needed by the
 * RequestTypePanel which uses the DBObject system for changing a record in the database file.
 ...
 * <br>
 * The RequestType should be replaced with capitalized version (class name) for the file.
 * requestType should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeRequestType(list of data args) to set the local variables to
 ...
 * FUtil.getEnum(databaseConnectionThread.getConnection("DATABASE"),"table","fieldName");
 * <br>
 ...
 * @author D.C. Ketchum
 */
public final class RequestType {   //implements Comparator
  // static ArrayList enumName;   // this need to be populated in makeRequestType()

  /**
   * Creates a new instance of RequestType
   */
  private int ID;                   // This is the database ID (should alwas be named "ID"
  private String requestType;             // This is the main key which should have same name
  ...
  private String cwbIP;
  private int cwbPort;
  private String edgeIP;
  private int edgePort;
  private String tablename;

  // Put in correct detail constructor here.  Use makeRequestType() by filling in all
   ...
   * is down
   */
  public RequestType(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeRequestType(rs.getInt("ID"), rs.getString("requestType"), rs.getString("requestclass"),
            rs.getString("cwbip"), rs.getInt("cwbport"), rs.getString("edgeip"), rs.getInt("edgeport"),
            rs.getString("tablename")
    // ,rs.getDouble(longitude)
    );
  }

  // Detail Constructor, match set up with makeRequestType(), this argument list should be
  // the same as for the result set builder as both use makeRequestType()
  /**
   ...
   * @param cl request class
   * @param cip CWB IP adr
   * @param cport CWB port.
   * @param eip edge IP adr
   * @param eport edge port
   * @param tbl fetch table name
   */
  public RequestType(int inID, String loc //USER: add fields, double lon
          ,
           String cl, String cip, int cport, String eip, int eport, String tbl
  ) {
    makeRequestType(inID, loc, cl, cip, cport, eip, eport, tbl //, lon
    );
  }
   ...
   *
   */
  private void makeRequestType(int inID, String loc //USER: add fields, double lon
          ,
           String cl, String cip, int cport, String eip, int eport, String tbl
  ) {
    ID = inID;
    requestType = loc;     // longitude = lon
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((RequestType) a).getRequestType().compareTo(((RequestType) b).getRequestType());
    }
    public boolean equals(Object a, Object b) {
      if( ((RequestType)a).getRequestType().equals( ((RequestType) b).getRequestType())) return true;
      return false;
    }
//  }*/
}
