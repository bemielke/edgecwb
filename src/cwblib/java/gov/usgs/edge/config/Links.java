...
/**
 * Links.java If the name of the class does not match the name of the field in the class you may
 * need to modify the rs.getSTring in the constructor.
 *
 * This Links templace for creating a MySQL database object. It is not really needed by the
 * LinksPanel which uses the DBObject system for changing a record in the database file. However,
 ...
 * <br>
 * The Links should be replaced with capitalized version (class name) for the file. links should be
 * replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeLinks(list of data args) to set the local variables to the
 ...
 * @author D.C. Ketchum
 */
public final class Links {    //implements Comparator
  // static ArrayList enumName;   // this need to be populated in makeLinks()

  /**
   * Creates a new instance of Links
   */
  private int ID;                   // This is the MySQL ID (should alwas be named "ID"
  private String links;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeLinks() by filling in all
   ...
   * is down
   */
  public Links(ResultSet rs) throws SQLException {
    makeLinks(rs.getInt("ID"), rs.getString("links"), rs.getString("description"), rs.getInt("hasdata")
    // ,rs.getDouble(longitude)
    );
  }

  // Detail Constructor, match set up with makeLinks(), this argument list should be
  // the same as for the result set builder as both use makeLinks()
  /**
   ...
   */
  public Links(int inID, String loc, String desc, int has //, double lon
  ) {
    makeLinks(inID, loc, desc, has //, lon
    );
  }
   ...
   *
   */
  private void makeLinks(int inID, String loc, String desc, int has //, double lon
  ) {
    ID = inID;
    links = loc;     // longitude = lon
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((Links) a).getLinks().compareTo(((Links) b).getLinks());
    }
    public boolean equals(Object a, Object b) {
      if( ((Links)a).getLinks().equals( ((Links) b).getLinks())) return true;
      return false;
    }
//  }*/
}
