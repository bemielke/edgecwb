 ...
 *
 * This Groups templace for creating a database database object.  It is not
 ...
 * @author  D.C. Ketchum
 */
public final class Groups {    //implements Comparator
   // static ArrayList enumName;   // this need to be populated in makeGroup()
  ...
  // Put in correct detail constructor here.  Use makeGroup() by filling in all
  ...
  public Groups(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeGroups(rs.getInt("ID"), rs.getString("groups"),
  ...
  // Detail Constructor, match set up with makeGroup(), this argument list should be
  // the same as for the result set builder as both use makeGroup()
   ...
   */
  public Groups(int inID, String loc   //USER: add fields, double lon
          , String inDesc
    ) {
    makeGroups(inID, loc, inDesc      //, lon
    );
  }
   ...
   **/
  private void makeGroups(int inID, String loc    //USER: add fields, double lon
          , String inDesc
  ) {
