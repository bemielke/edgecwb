...
/**
 * This allows two different server ports to talk to this server and exchanged data. It opens a
 * socket to both servers and sends all received bytes to the other socket. No known uses at this
 * time.
 *
 * @author U.S. Geological Survey <ketchum at usgs.gov>
 */
public final class ClientClientCopier extends Thread {
  ...
  private boolean dbg;
  private final ShutdownClientClient shutdown;

  /**
   * write packets from queu
   *
   * @param argline
   * @param tg
   */
    ...
    for (int i = 0; i < args.length; i++) {
      switch (args[i]) {
        ...
        case "-hout":
          ...
          break;
        case "-pin":
          portin = Integer.parseInt(args[i + 1]);
          i++;
          break;
        case "-pout":
          portout = Integer.parseInt(args[i + 1]);
          i++;
          break;
        case "-dbg":
          dbg = true;
        ...
        case "-tag":
          ...
          break;
        default:
          Util.prta(tag + " unknown switch " + i + " = " + args[i]);
          break;
      }
    }
  ...
  public final class CheckProgress extends Thread {
