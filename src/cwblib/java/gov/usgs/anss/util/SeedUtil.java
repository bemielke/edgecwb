...
/**
 * This class is all static methods for manipulating julian days, day-of-year, back and forth. Here
 * lie all of the static functions needed as Utilities for the Edge and seed files. The Julian day
 * routines can make the calculation a year end meaningless.
 *
 *
 * @author davidketchum
 */
public final class SeedUtil {
  ...
  private static final int[] daytab = new int[]{0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
  private static final int[] dayleap = new int[]{0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
  private static final GregorianCalendar cal = new GregorianCalendar(TimeZone.getTimeZone("GMT+0000"));

  /**
   * From a year, month, and day of the month calculate the Julian day of year. Parts stolen from
   * K&R's C book.
   *
 ...
 /*	printf("Mon=%s month=%d leap=%d\n",mn,month,leap);*/
   ...
   *
   * @param yr The year
   * @param mon a three digit Man (first 3 letters of English month
 ...
 /*	printf("Mon=%s month=%d leap=%d\n",mn,month,leap);*/
  ...
  /**
   * From a Unix 3 character month in ascii, a day of month, and year, calculate the Julian day of
   ...
   * @param yr The year
   * @param doy The day of the year
   * @param ymd THe user supplied array to put the ymd in
   * @return an array in yr, mon, day
   * @throws RuntimeException ill formed especially doy being too big.
   */
  public static int[] ymd_from_doy(int yr, int doy, int[] ymd) throws RuntimeException {
    int j;
    int sum;
    yr = sanitizeYear(yr);
    boolean leap = yr % 4 == 0 && yr % 100 != 0 || yr % 400 == 0;
  ...
  /**
   * Sanitize year using the rule of 60, two digit ears >=60 = 1900+yr years less than 60 are
   * 2000+yr. If its 4 digits already, just return it after testing its in the range 1900 - 2500
   *
  ...
  public static int sanitizeYear(int yr) {
    if (yr >= 100) {
      if(yr < 1800) { // Filter our outrageous values
        System.out.println("** Illegal year to sanitize (set to 1800) =" + yr);
        yr=1800;
        
      }     
      if(yr > 2500) {// filter out really outrageous values
        System.out.println("**Illegal year to sanitize (set to 2500) =" + yr);
        yr=2500;
      }
      return yr;
    }
   ...
   * negative year B.C. Remember that the year after 1 B.C. was 1 A.D. This is good for translating
   * day differences because it will calculate the calendar days correctly.
   *
  ...
  /**
   * Convert a epoch millis to a julian day
   *
   * @param date the date is epoch in millis since 1970
   * @return the Julian date
   */
  public static int toJulian(long date) {
    int jul;
    synchronized (cal) {
  ...
  /**
   * Converts a Julian day to a calendar date in a ymd array ref : Numerical Recipes in C, 2nd ed.,
   * Cambridge University Press 1992
   *
   * @param injulian The julian date (a big number!)
   * @param ymd User array to receive the year, month and day
   */
  public static void fromJulian(int injulian, int[] ymd) {
  ...
  public static String fileStub(int year, int jday) {
    ...
    return Util.df4(sanitizeYear(year)) + "_" + Util.df3(jday);
  }
  ...
  public static void main(String args[]) {
    ...
    int todayJulian = toJulian(today.get(Calendar.YEAR), today.get(Calendar.MONTH) + 1,
