...
public class Steim2 {
  ...
  public static String getDebugString() {return (sb == null?"null":sb.toString());}
  ...
  static public int getRIC() {return end;}
	/**
  ...
  public static int[] decode(byte[] b, int numSamples, boolean swapBytes, int bias) throws SteimException {
	 ...
	 *  @param numSamples the number of samples that can be decoded from array
	 *  <b>b</b>
   *  @param samples User provided array of at least numSamples length to put data
	 *  @param swapBytes if true, swap reverse the endian-ness of the elements of
	 *  byte array <b>b</b>.
	 *  @param bias the first difference value will be computed from this value.
	 *  If set to 0, the method will attempt to use the X(0) constant instead.
	 ...
	 *  bytes.
	 */	
  public synchronized static int[] decode(byte[] b, int numSamples, int [] samples, boolean swapBytes, int bias) throws SteimException {
		if (b.length % 64 != 0) {
   ...
   * @param swapBytes If true, bytes are swapped
   * @return  The data samples
   * @throws gov.usgs.anss.seed.SteimException
   * @param b byte buffer with the compressed data
	 */
    ...
    if(dbg) sb.append("Key=").append(Integer.toHexString(nibbles)).append("\n");
		...
		for (int i=0; i<16; i++) {
          ...
          if(dbg) sb.append("Word ").append(i).append(" is 8 byte ns=").append(currNum - 4).
                  ...
                  append(temp[currNum - 4]).append(" ").append(temp[currNum - 3]).
                  append(" ").append(temp[currNum - 2]).append(" ").append(temp[currNum - 1]).append("\n");
					break;
				case 2:
              ...
              if(dbg) sb.append("Word ").append(i).append(" is 02 dnib=").
                      append(dnib).append(" ns=").append(currNum - 2).append("diff(15 bit)=").
