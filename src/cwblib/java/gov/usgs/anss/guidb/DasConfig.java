 ...
 *
 * This DasConfig templace for creating a database database object.  It is not
 * really needed by the DasConfigPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The DasConfig should be replaced with capitalized version (class name) for the 
 * file.  dasconfig should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeDasConfig(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class DasConfig     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeDasConfig()
  
  /** Creates a new instance of DasConfig */
  int ID;                   // This is the database ID (should alwas be named "ID"
  String dasconfig;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeDasConfig() by filling in all
   ...
   */
  public DasConfig(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeDasConfig(rs.getInt("ID"), 
        rs.getString("dasconfig"),
        rs.getString("text"),
        rs.getString("filltype"),
        rs.getString("expectedchannels")
                            // ,rs.getDouble(longitude)
    );
  }
  
  // Detail Constructor, match set up with makeDasConfig(), this argument list should be
  // the same as for the result set builder as both use makeDasConfig()
   ...
   *@param loc The key value for the database (sam
   * @param txt description of the configuration
   * @param fill The fill type (2 char)
   * @param exp The list of expected channels (e.g. BH110, BH210,BHZ10,LH1,LH2,LHZ)
   */
  public DasConfig(int inID, String loc, String txt, String fill,String exp   //USER: add fields, double lon
    ) {
    makeDasConfig(inID, loc  ,txt ,fill,exp    //, lon
    );
  }
   ...
   **/
  private void makeDasConfig(int inID, String loc ,String txt, String fill, String exp   //USER: add fields, double lon
  ) {
    ID = inID;  dasconfig=loc;     // longitude = lon
   ...
   *@return the key name */
  public String toString2() { return dasconfig;}
  /** return the key name as the string
   *@return the key name */
  @Override
  public String toString() { return dasconfig;}
  // getter
   ...
   */
  public String getDasConfig() {return dasconfig;}
  
  
  //USER:  All field getters here
  //  public double getLongitude() { return longitude;}
  public String getText() {return text;}
  public String getFillType() {return fillGroup;}
  public String getExpectedChannels() {return expected;}
  
  // This is needed if you need to implement Comparator
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((DasConfig) a).getDasConfig().compareTo(((DasConfig) b).getDasConfig());
    }
    public boolean equals(Object a, Object b) {
      if( ((DasConfig)a).getDasConfig().equals( ((DasConfig) b).getDasConfig())) return true;
      return false;
    }
//  }*/
