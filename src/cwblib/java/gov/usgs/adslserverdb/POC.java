 ...
 *
 * This POC templace for creating a MySQL database object.  It is not
 * really needed by the POCPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The POC should be replaced with capitalized version (class name) for the
 * file.  poc should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makePOC(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class POC     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makePOC()
  
  /** Creates a new instance of POC */
  int ID;                   // This is the MySQL ID (should alwas be named "ID"
  String poc;             // This is the main key which should have same name
                            // as the table which it is a key to.  That is
                            // Table "category" should have key "category"
  
  // USER: All fields of file go here
  //  double longitude
  String email, phone, cellphone,comment;

  
  // Put in correct detail constructor here.  Use makePOC() by filling in all
   ...
   */
  public POC(ResultSet rs) throws SQLException {  //USER: add all data field here
    makePOC(rs.getInt("ID"), rs.getString("poc"),
            rs.getString("email"), rs.getString("phone"), rs.getString("cellphone"), rs.getString("comment")
                            // ,rs.getDouble(longitude)
    );
  }
  
  // Detail Constructor, match set up with makePOC(), this argument list should be
  // the same as for the result set builder as both use makePOC()
   ...
   */
  public POC(int inID, String loc   //USER: add fields, double lon
          , String em, String ph, String cell, String cmnt
    ) {
    makePOC(inID, loc ,em, ph, cell, cmnt     //, lon
    );
  }
   ...
   **/
  private void makePOC(int inID, String loc    //USER: add fields, double lon
          , String em, String ph, String cell, String cmnt
  ) {
    ID = inID;  poc=loc;     // longitude = lon
    //USER:  Put asssignments to all fields from arguments here
    email = em; phone=ph; cellphone=cell; comment=cmnt;
    // ENUM example:
   ...
   *@return the key name */
  public String toString2() { return poc;}
  /** return the key name as the string
   *@return the key name */
  @Override
  public String toString() { return poc;}
  // getter
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((POC) a).getPOC().compareTo(((POC) b).getPOC());
    }
    public boolean equals(Object a, Object b) {
      if( ((POC)a).getPOC().equals( ((POC) b).getPOC())) return true;
      return false;
    }
//  }*/
