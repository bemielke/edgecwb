 ...
 *
 * This NeicAgency templace for creating a MySQL database object.  It is not
 * really needed by the NeicAgencyPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The NeicAgency should be replaced with capitalized version (class name) for the
 * file.  neicAgency should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeNeicAgency(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class NEICAgency     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeNeicAgency()
  
  /** Creates a new instance of NeicAgency */
  int ID;                   // This is the MySQL ID (should alwas be named "ID"
  String neicAgency;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeNeicAgency() by filling in all
   ...
   */
  public NEICAgency(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeNeicAgency(rs.getInt("ID"), rs.getString("neicagency"), rs.getInt("agencyid"), rs.getInt("deploymentid")
                            // ,rs.getDouble(longitude)
    );
  }
  
  // Detail Constructor, match set up with makeNeicAgency(), this argument list should be
  // the same as for the result set builder as both use makeNeicAgency()
   ...
   */
  public NEICAgency(int inID, String loc, int aID, int dID   //USER: add fields, double lon
    ) {
    makeNeicAgency(inID, loc, aID, dID       //, lon
    );
  }
   ...
   **/
  private void makeNeicAgency(int inID, String loc , int aID, int dID   //USER: add fields, double lon
  ) {
    ID = inID;  neicAgency=loc;     // longitude = lon
    ...
    agencyID=aID; deploymentID = dID;
    
  }
  /** return the key name as the string
   *@return the key name */
  public String toString2() { return neicAgency;}
  /** return the key name as the string
   *@return the key name */
  @Override
  public String toString() { return neicAgency;}
  // getter
   ...
   */
  public String getNEICAgency() {return neicAgency;}
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((NeicAgency) a).getNeicAgency().compareTo(((NeicAgency) b).getNeicAgency());
    }
    public boolean equals(Object a, Object b) {
      if( ((NeicAgency)a).getNeicAgency().equals( ((NeicAgency) b).getNeicAgency())) return true;
      return false;
    }
//  }*/
