 ...
 *
 * This Stationflags templace for creating a MySQL database object.  It is not
 * really needed by the StationflagsPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The Stationflags should be replaced with capitalized version (class name) for the 
 * file.  flags should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeStationflags(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class StationFlags implements Maskable  //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeStationstationflags()
  
  /** Creates a new instance of Stationflags */
  int ID;                   // This is the MySQL ID (should alwas be named "ID"
  String stationflags;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeStationflags() by filling in all
   ...
   */
  public StationFlags(ResultSet rs) throws SQLException {
    makeStationflags(rs.getInt("ID"), rs.getString("stationflags"), rs.getString("description"), rs.getInt("hasdata")
                            // ,rs.getDouble(longitude)
    );
  }
  
  // Detail Constructor, match set up with makeStationflags(), this argument list should be
  // the same as for the result set builder as both use makeStationflags()
   ...
   */
  public StationFlags(int inID, String loc, String desc , int has  //, double lon
    ) {
    makeStationflags(inID, loc, desc ,has      //, lon
    );
  }
   ...
   **/
  private void makeStationflags(int inID, String loc, String desc, int has    //, double lon
  ) {
    ID = inID;  stationflags=loc;     // longitude = lon
   ...
   *@return the key name */
  public String toString2() { return stationflags;}
  /** return the key name as the string
   *@return the key name */
  @Override
  public String toString() { return stationflags;}
  // getter
   ...
   */
  public String getStationflags() {return stationflags;}
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((Stationflags) a).getStationflags().compareTo(((Stationflags) b).getStationflags());
    }
    public boolean equals(Object a, Object b) {
      if( ((Stationflags)a).getStationflags().equals( ((Stationflags) b).getStationflags())) return true;
      return false;
    }
//  }*/
