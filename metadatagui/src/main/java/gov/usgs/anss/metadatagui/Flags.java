 ...
 *
 * This Flags templace for creating a MySQL database object.  It is not
 * really needed by the FlagsPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The Flags should be replaced with capitalized version (class name) for the 
 * file.  flags should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeFlags(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class Flags implements Maskable     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeFlags()
  ...
  String flags;             // This is the main key which should have same name
  ...
  private void makeFlags(int inID, String loc, String desc, int has    //, double lon
    ...
    ID = inID;  flags=loc;     // longitude = lon
   ...
   *@return the key name */
  public String toString2() { return flags;}
  /** return the key name as the string
   *@return the key name */
  @Override
