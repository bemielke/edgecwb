 ...
 *
 * This Channelflags templace for creating a MySQL database object.  It is not
 * really needed by the ChannelflagsPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The Channelflags should be replaced with capitalized version (class name) for the 
 * file.  flags should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeChannelflags(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class ChannelFlags implements Maskable     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeChannelflags()
  
  /** Creates a new instance of Channelflags */
  int ID;                   // This is the MySQL ID (should alwas be named "ID"
  String channelflags;             // This is the main key which should have same name
   ...
   */
  public ChannelFlags(ResultSet rs) throws SQLException {
    makeChannelflags(rs.getInt("ID"), rs.getString("channelflags"), rs.getString("description"), rs.getInt("hasdata")
                            // ,rs.getDouble(longitude)
    );
  }
  
  // Detail Constructor, match set up with makeChannelflags(), this argument list should be
  // the same as for the result set builder as both use makeChannelflags()
   ...
   */
  public ChannelFlags(int inID, String loc, String desc , int has  //, double lon
    ) {
    makeChannelflags(inID, loc, desc ,has      //, lon
    );
  }
   ...
   **/
  private void makeChannelflags(int inID, String loc, String desc, int has    //, double lon
  ) {
    ID = inID;  channelflags=loc;     // longitude = lon
   ...
   *@return the key name */
  public String toString2() { return channelflags;}
  /** return the key name as the string
   *@return the key name */
  @Override
  public String toString() { return channelflags;}
  // getter
   ...
   */
  public String getChannelflags() {return channelflags;}
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((Channelflags) a).getChannelflags().compareTo(((Channelflags) b).getChannelflags());
    }
    public boolean equals(Object a, Object b) {
      if( ((Channelflags)a).getChannelflags().equals( ((Channelflags) b).getChannelflags())) return true;
      return false;
    }
//  }*/
