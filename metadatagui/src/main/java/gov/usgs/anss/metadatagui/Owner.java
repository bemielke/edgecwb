 ...
 * @author  D.C. Ketchum
 */
public class Owner implements Maskable     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeOwner()
  ...
  String owner;             // This is the main key which should have same name
  ...
  public Owner(ResultSet rs) throws SQLException {
    makeOwner(rs.getInt("ID"), rs.getString("owner"), rs.getString("description"), 
   ...
   */
  public Owner(int inID, String loc, String desc , String con, String add  //, double lon
    ) {
    makeOwner(inID, loc, desc ,con, add      //, lon
    );
  }
   ...
   **/
  private void makeOwner(int inID, String loc, String desc, String con, String add    //, double lon
  ) {
    ID = inID;  owner=loc;     // longitude = lon
   ...
   *@return the key name */
  public String toString2() { return owner;}
  /** return the key name as the string
   *@return the key name */
  @Override
