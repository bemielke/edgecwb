 ...
 *
 * This StatusLog templace for creating a MySQL database object.  It is not
 * really needed by the StatusLogPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The StatusLog should be replaced with capitalized version (class name) for the 
 * file.  statuslog should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeStatusLog(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class StatusLog     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeStatusLog()
  
  /** Creates a new instance of StatusLog */
  int ID;                   // This is the MySQL ID (should alwas be named "ID"
  String statuslog;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeStatusLog() by filling in all
   ...
   */
  public StatusLog(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeStatusLog(rs.getInt("ID"), rs.getString("station"), rs.getString("date"),rs.getString("bin")
                            // ,rs.getDouble(longitude)
    );
  }
  
  // Detail Constructor, match set up with makeStatusLog(), this argument list should be
  // the same as for the result set builder as both use makeStatusLog()
   ...
   */
  public StatusLog(int inID, String loc, String dat, String blob   //USER: add fields, double lon
    ) {
    makeStatusLog(inID, loc, dat, blob       //, lon
    );
  }
   ...
   **/
  private void makeStatusLog(int inID, String loc , String dat, String blob   //USER: add fields, double lon
  ) {
    ID = inID;  statuslog=loc;     // longitude = lon
   ...
   *@return the key name */
  public String toString2() { return statuslog;}
  /** return the key name as the string
   *@return the key name */
  @Override
  public String toString() { return statuslog;}
  // getter
   ...
   */
  public String getStation() {return statuslog;}
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((StatusLog) a).getStatusLog().compareTo(((StatusLog) b).getStatusLog());
    }
    public boolean equals(Object a, Object b) {
      if( ((StatusLog)a).getStatusLog().equals( ((StatusLog) b).getStatusLog())) return true;
      return false;
    }
//  }*/
