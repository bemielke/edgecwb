...
//  FlagOnOffPanel2 sendtoPanel;
  private FlagOnOffPanel2 flagsPanel;
...
//    UC.Look(fixed);
  ...
  private void initComponents() {
    ...
    jScrollPane.setMinimumSize(new java.awt.Dimension(700, 250));
    jScrollPane.setPreferredSize(new java.awt.Dimension(700, 275));
    ...
    stationnames.setFont(new java.awt.Font("Courier", 0, 13)); // NOI18N
...
//    if(regexpStation.getText().equals("")) return;
    try {
      Connection C = DBConnectionThread.getConnection("status");
      ...
      if (reg.length()<=0){
        rs = stmt.executeQuery("SELECT station,date,uncompress(bin) FROM status.log "
          ...
          rs = stmt.executeQuery("SELECT * FROM log "+reg+" ORDER BY station");
        }
      else {
        // NOTE: The following result set is accessed by column later, be careful when modifying
        rs = stmt.executeQuery("SELECT station,date,uncompress(bin) FROM log WHERE station REGEXP '"+
      ...
      while (rs.next()) {
        ...
        for (String line1 : line) {
          match=false;
          if (regexp.equals("") & line1.contains(glob)) {
            match=true;
          } else if (line1.matches(regexp)) {
            match=true;
          }
          if (match) {
            if (!filenameprinted){
            ...
            if (linecount>=10000) {
              ...
              sb2.append("  - ").append(line1).append("\n");
              linecount++;
            }
          }
        }
