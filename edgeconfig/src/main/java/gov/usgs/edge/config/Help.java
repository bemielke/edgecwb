 ...
 *
 * This Help templace for creating a database database object.  It is not
 * really needed by the HelpPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The Help should be replaced with capitalized version (class name) for the 
 * file.  help should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeHelp(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class Help     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeHelp()
  ...
  String help;             // This is the main key which should have same name
   ...
   */
  public Help(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeHelp(rs.getInt("ID"), rs.getString("help"), rs.getString("description")
   ...
   */
  public Help(int inID, String loc , String indescription  //USER: add fields, double lon
    ) {
    makeHelp(inID, loc, indescription       //, lon
    );
  }
   ...
   **/
  private void makeHelp(int inID, String loc, String indescription    //USER: add fields, double lon
  ) {
    ID = inID;  help=loc; description=indescription;    // longitude = lon
   ...
   *@return the key name */
  public String toString2() { return help;}
  /** return the key name as the string
   *@return the key name */
  @Override
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((Help) a).getHelp().compareTo(((Help) b).getHelp());
    }
    public boolean equals(Object a, Object b) {
      if( ((Help)a).getHelp().equals( ((Help) b).getHelp())) return true;
      return false;
    }
//  }*/
