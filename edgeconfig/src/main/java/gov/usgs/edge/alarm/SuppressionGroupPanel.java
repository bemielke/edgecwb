 ...
 * JComboBox variable this variable is called "suppressiongroup" in the initial form
 * The table name and key name should match and start lower case (suppressiongroup).
 * The Class here will be that same name but with upper case at beginning(SuppressionGroup).
 * <br> 1)  Rename the location JComboBox to the "key" (suppressiongroup) value.  This should start Lower
 *      case.
 * <br>  2)  The table name should match this key name and be all lower case.
 * <br>  3)  Change all SuppressionGroup to ClassName of underlying data (case sensitive!)
 * <br>  4)  Change all suppressiongroup key value (the JComboBox above)
 ...
 * <br> 10) SuppressionGroupPanel() constructor - good place to change backgrounds using UC.Look() any
 ...
 * @author D.C. Ketchum
 */
public final class SuppressionGroupPanel extends javax.swing.JPanel {
  
  // NOTE : here define all variables general.  "ArrayList suppressionGroups" is used for main Comboboz
  static ArrayList<SuppressionGroup> suppressionGroups;             // ArrayList containing objects of this SuppressionGroup Type
  ...
  private boolean chkForm() {
   ...
   Util.prt("chkForm SuppressionGroup");
  ...
  private void clearScreen() {
    ...
    addUpdate.setText("Enter a SuppressionGroup");
...
//    )
  ...
  private SuppressionGroup newOne() {
      Timestamp t = new Timestamp(0l);
    return new SuppressionGroup(0, ((String) suppressiongroup.getSelectedItem()).toUpperCase() //, more
  ...
  private void oldOne(String p) throws SQLException {
    ...
    "alarm", "supression","suppression",p);
    ...
    if(obj.isNew()) {
      ...
      //      suppressiongroup.setText(obj.getString("SuppressionGroup"));
  ...
  private void initComponents() {
    ...
    jLabel1.setText("SuppressionGroup : ");
    ...
    suppressiongroup.setEditable(true);
    suppressiongroup.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(java.awt.event.ActionEvent evt) {
        suppressiongroupActionPerformed(evt);
      }
    });
    ...
    add(suppressiongroup, gridBagConstraints);
  ...
  private void addUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addUpdateActionPerformed
    ...
    if(chkForm()) return;
    try {
      String p = suppressiongroup.getSelectedItem().toString();
      ...
      obj.setString("suppressiongroup",p);
      ...
      // Do not change
      obj.updateRecord();
      suppressionGroups=null;       // force reload of combo box
      getJComboBox(suppressiongroup);
      clearScreen();
      suppressiongroup.setSelectedIndex(-1);
    }  catch  (SQLException  E)
    { Util.SQLErrorPrint(E,"suppressiongroup: update failed partno="+suppressiongroup.getSelectedItem());
  ...
  private void suppressiongroupActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_suppressiongroupActionPerformed
    // Add your handling code here:
   find();
 
 
  }//GEN-LAST:event_suppressiongroupActionPerformed
  ...
  private javax.swing.JComboBox suppressiongroup;
  // End of variables declaration//GEN-END:variables
  /**
   * Creates new form SuppressionGroupPanel
   */
  public SuppressionGroupPanel() {
    initiating=true;
    initComponents();
    getJComboBox(suppressiongroup);                // set up the key JComboBox
    suppressiongroup.setSelectedIndex(-1);    // Set selected type
 ...
 public static JComboBox getJComboBox() { JComboBox b = new JComboBox(); getJComboBox(b); return b;}
  ...
  public static void getJComboBox(JComboBox b) {
    b.removeAllItems();
    makeSuppressionGroups();
    for (int i=0; i< suppressionGroups.size(); i++) {
      b.addItem(suppressionGroups.get(i));
    }
  ...
  public static void setJComboBoxToID(JComboBox b, int ID) {
    ...
    //Util.prt("SuppressionGroupPanel.setJComboBoxToID id="+ID);
    for(int i=0; i<b.getItemCount(); i++) 
      if(((SuppressionGroup) b.getItemAt(i)).getID() == ID) {
          b.setSelectedIndex(i);
          //Util.prt("SuppressionGroupPanel.setJComboBoxToID id="+ID+" found at "+i);
      }
  }  
  ...
  public static int getJComboBoxIndex(int ID) {
    if(suppressionGroups == null) makeSuppressionGroups();
    for(int i=0; i<suppressionGroups.size(); i++) if( ID == ((SuppressionGroup) suppressionGroups.get(i)).getID()) return i;
    return -1;
  }
  ...
  public static void reload() {
    ...
    /* return a ArrayList with all of the SuppressionGroup
   * @return The ArrayList with the suppressiongroup
   */
  public static ArrayList<SuppressionGroup> getSuppressionGroupArrayList() {
    if(suppressionGroups == null) makeSuppressionGroups();
    return suppressionGroups;
  }
  /** Get the item corresponding to database ID
   *@param ID the database Row ID
   *@return The SuppressionGroup row with this ID
   */
  public static SuppressionGroup getItemWithID(int ID) {
    if(suppressionGroups == null) makeSuppressionGroups();
    int i=getJComboBoxIndex(ID);
    if(i >= 0) return (SuppressionGroup) suppressionGroups.get(i);
    else return null;
  }
  
  // This routine should only need tweeking if key field is not same as table name
  private static void makeSuppressionGroups() {
    if (suppressionGroups != null) return;
    suppressionGroups=new ArrayList<SuppressionGroup>(100);
    try {
      Statement stmt = DBConnectionThread.getConnection(DBConnectionThread.getDBSchema()).createStatement();   // used for query
      String s = "SELECT * FROM alarm.suppressiongroup ORDER BY suppressiongroup;";
      ResultSet rs = stmt.executeQuery(s);
      while (rs.next()) {
        SuppressionGroup loc = new SuppressionGroup(rs);
//        Util.prt("MakeSuppressionGroup() i="+suppressionGroups.size()+" is "+loc.getSuppressionGroup());
        suppressionGroups.add(loc);
      }
    }
    catch (SQLException e) {
      Util.SQLErrorPrint(e,"makeSuppressionGroup() on table SQL failed");
    }    
  }
  
  // No changes needed
  private void find() {
    if(suppressiongroup == null) return;
    if(initiating) return;
    SuppressionGroup l;
    if(suppressiongroup.getSelectedIndex() == -1) {
      if(suppressiongroup.getSelectedItem() == null) return;
      l = newOne();
    } 
    else {
      l = (SuppressionGroup) suppressiongroup.getSelectedItem();
    }
      
    if(l == null) return;
    String p = l.getSuppressionGroup();
    if(p == null) return;
    if(p.equals("")) {
      addUpdate.setEnabled(false);
      addUpdate.setText("Enter a SuppressionGroup!");
    ...
    { Util.SQLErrorPrint(E,"SuppressionGroup: SQL error getting SuppressionGroup="+p);
    }
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      Show.inFrame(new SuppressionGroupPanel(), UC.XSIZE, UC.YSIZE);
    }
