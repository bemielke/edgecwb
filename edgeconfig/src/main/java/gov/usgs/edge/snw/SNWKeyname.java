...
/**
 * SNWKeyname.java If the name of the class does not match the name of the field in the class you
 * may need to modify the rs.getSTring in the constructor.
 *
 * This SNWKeyname templace for creating a MySQL database object. It is not really needed by the
 * SNWKeynamePanel which uses the DBObject system for changing a record in the database file.
 ...
 * <br>
 * The SNWKeyname should be replaced with capitalized version (class name) for the file. snwkeyname
 * should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeSNWKeyname(list of data args) to set the local variables to
 ...
 * @author D.C. Ketchum
 */
public final class SNWKeyname {    //implements Comparator
  // static ArrayList enumName;   // this need to be populated in makeSNWKeyname()

  /**
   * Creates a new instance of SNWKeyname
   */
  private int ID;                   // This is the MySQL ID (should alwas be named "ID"
  private String snwkeyname;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeSNWKeyname() by filling in all
   ...
   * is down
   */
  public SNWKeyname(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeSNWKeyname(rs.getInt("ID"),
            rs.getString("snwkeyname"),
            rs.getString("description")
    // ,rs.getDouble(longitude)
    );
  }

  // Detail Constructor, match set up with makeSNWKeyname(), this argument list should be
  // the same as for the result set builder as both use makeSNWKeyname()
  /**
   ...
   * @param description
   */
  public SNWKeyname(int inID, String loc, String description //USER: add fields, double lon
  ) {
    makeSNWKeyname(inID, loc, "" //, lon
    );
  }
   ...
   *
   */
  private void makeSNWKeyname(int inID, String loc, String desc //USER: add fields, double lon
  ) {
    ID = inID;
    snwkeyname = loc;     // longitude = lon
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((SNWKeyname) a).getSNWKeyname().compareTo(((SNWKeyname) b).getSNWKeyname());
    }
    public boolean equals(Object a, Object b) {
      if( ((SNWKeyname)a).getSNWKeyname().equals( ((SNWKeyname) b).getSNWKeyname())) return true;
      return false;
    }
//  }*/
}
