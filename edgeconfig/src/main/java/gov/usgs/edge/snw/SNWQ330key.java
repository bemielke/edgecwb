...
/**
 * SNWQ330key.java If the name of the class does not match the name of the field in the class you
 * may need to modify the rs.getSTring in the constructor.
 *
 * This SNWQ330key templace for creating a MySQL database object. It is not really needed by the
 * SNWQ330keyPanel which uses the DBObject system for changing a record in the database file.
 ...
 * <br>
 * The SNWQ330key should be replaced with capitalized version (class name) for the file. snwq330key
 * should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeSNWQ330key(list of data args) to set the local variables to
 ...
 * @author D.C. Ketchum
 */
public final class SNWQ330key {    //implements Comparator
  // static ArrayList enumName;   // this need to be populated in makeSNWQ330key()

  /**
   * Creates a new instance of SNWQ330key
   */
  private int ID;                   // This is the MySQL ID (should alwas be named "ID"
  private String snwq330key;             // This is the main key which should have same name
  ...
  //  double longitude
  // Put in correct detail constructor here.  Use makeSNWQ330key() by filling in all
   ...
   * is down
   */
  public SNWQ330key(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeSNWQ330key(rs.getInt("ID"), rs.getString("snwq330key"), rs.getString("description")
    // ,rs.getDouble(longitude)
    );
  }

  // Detail Constructor, match set up with makeSNWQ330key(), this argument list should be
  // the same as for the result set builder as both use makeSNWQ330key()
  /**
   ...
   * @param desc
   */
  public SNWQ330key(int inID, String loc, String desc //USER: add fields, double lon
  ) {
    makeSNWQ330key(inID, loc, desc //, lon
    );
  }
   ...
   *
   */
  private void makeSNWQ330key(int inID, String loc, String desc //USER: add fields, double lon
  ) {
    ID = inID;
    snwq330key = loc;
    description = desc;   // longitude = lon
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((SNWQ330key) a).getSNWQ330key().compareTo(((SNWQ330key) b).getSNWQ330key());
    }
    public boolean equals(Object a, Object b) {
      if( ((SNWQ330key)a).getSNWQ330key().equals( ((SNWQ330key) b).getSNWQ330key())) return true;
      return false;
    }
//  }*/
}
