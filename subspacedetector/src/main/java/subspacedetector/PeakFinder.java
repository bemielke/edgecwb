...
/**
 * A static function to find peaks in a time series.
 *
...
public final class PeakFinder {
   ...
   * @param width The width of the minimum window size (array elements before retriggering is
   * possible)
   ...
   * @return The number of elements in picks[]
   */
  public static int findPicks(float[] data, int nsamp, int width, double minCorrelation,
          double[] hist, int[] picks, StringBuilder err) {
    int npick = 0;
    if (hist.length < width + nsamp) {
      Util.clear(err);
  ...
  public static int findPicks(double[] data, int offset, int nsamp, int width,
    ...
    while (i < npick) {
      ...
      i = lastk + 1;
      if (picks[np] < offset + nsamp + width / 2 && picks[np] >= offset + width / 2) {
        np++;
      }
    }
    return np;
   ...
   * @return The number of elements in picks[], if less than 0 then an error is being reported
   */
  //JN: This is the findPicks being used
  public static int findPicks(float[] data, int offset, int nsamp, int width,
          double minCorrelation, int[] picks, StringBuilder err) {
    int npick = 0;
    for (int i = Math.max(0, offset - width); i < offset + nsamp; i++) {
      //System.out.println("\n\nMin. Correlation is: " + minCorrelation + " Current value is: " + data[i] + "\n\n");  
      if (data[i] >= minCorrelation) {
        if (npick >= picks.length - 1) {
          Util.clear(err);
          err.append("*** the picks array is too small - needs to be at least ").append(npick * 2);
    ...
    while (i < npick) {
      ...
      //DCK: this looks wrong if (picks[np] < offset + nsamp + width / 2 && picks[np] >= offset + width / 2 && picks[np] - lastpeak > width / 2) {
        lastpeak = picks[np];
        np++;
      }
    }
  ...
  public static void main(String[] args) {
    ...
    try {
      BufferedReader in = new BufferedReader(new FileReader("YHB_2013_327stack.dat"));
      String line = null;
      int totpick = 0;
      boolean dbg = false;
      int maxdata = 10240;
      ...
      int[] picks = new int[maxdata];
      GregorianCalendar g = new GregorianCalendar();
      g.set(2013, 10, 23, 0, 0, 0);
      ...
      for (;;) {
        ...
        for (i = 0; i < npick; i++) {
          ...
          sb.append(Util.ascdate(time)).append(" ").append(Util.asctime2(time)).append(" ").
                  ...
                  append(picks[i] >= 0 ? Util.df23.format(data[picks[i]]) : "LAST").append(" data=");
          if (dbg) {
            for (int j = Math.max(0, picks[i] - width / 2); j < Math.min(picks[i] + width / 2, maxdata - width); j++) {
              sb.append(j).append("=").append(Util.df23.format(data[j])).append(" ");
            }
          }
      ...
      g.set(2013, 10, 23, 0, 0, 0);
      g.setTimeInMillis(g.getTimeInMillis() / 1000 * 1000);
      base = g.getTimeInMillis();
      totpick = 0;
      // No rerun this using the nonhistory one.
      in = new BufferedReader(new FileReader("YHB_2013_327stack.dat"));
      off = 0;
      runtime = 0;
      for (;;) {
        int i = width;
