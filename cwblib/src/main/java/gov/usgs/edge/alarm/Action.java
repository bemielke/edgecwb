...
/**
 * Action.java If the name of the class does not match the name of the field in the class you may
 * need to modify the rs.getSTring in the constructor.
 *
 * This Action templace for creating a MySQL database object. It is not really needed by the
 * ActionPanel which uses the DBObject system for changing a record in the database file. However,
 ...
 * <br>
 * The Action should be replaced with capitalized version (class name) for the file. action should
 * be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeAction(list of data args) to set the local variables to the
 ...
 * @author D.C. Ketchum
 */
public final class Action //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeAction()

  /**
   * Creates a new instance of Action
   */
  private int ID;                   // This is the MySQL ID (should alwas be named "ID"
  private String action;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeAction() by filling in all
   ...
   * is down
   */
  public Action(ResultSet rs) throws SQLException {  //USER: add all data field here
    reload(rs);
  }
  ...
  public final void reload(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeAction(rs.getInt("ID"), rs.getString("action"),
            rs.getString("class"),
            rs.getString("args")
   ...
   * is down
   */
  public Action(DBTable rs) throws SQLException {  //USER: add all data field here
    reload(rs);
  }
   ...
   * @param inargs
   */
  public Action(int inID, String loc, String inaclass, String inargs //USER: add fields, double lon
  ) {
    makeAction(inID, loc, inaclass, inargs //, lon
    );
  }
   ...
   *
   */
  private void makeAction(int inID, String loc, String inaclass, String inargs //USER: add fields, double lon
  ) {
    //USER:  Put asssignments to all fields from arguments here
    ID = inID;
    action = loc;
    aclass = inaclass;
    args = inargs;     // longitude = lon
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((Action) a).getAction().compareTo(((Action) b).getAction());
    }
    public boolean equals(Object a, Object b) {
      if( ((Action)a).getAction().equals( ((Action) b).getAction())) return true;
      return false;
    }
//  }*/
}
