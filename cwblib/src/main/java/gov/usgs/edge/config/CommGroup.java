...
/**
 * CommGroup.java If the name of the class does not match the name of the field in the class you may
 * need to modify the rs.getSTring in the constructor.
 *
 * This CommGroup templace for creating a database database object. It is not really needed by the
 * CommGroupPanel which uses the DBObject system for changing a record in the database file.
 ...
 * <br>
 * The CommGroup should be replaced with capitalized version (class name) for the file. commgroup
 * should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeCommGroup(list of data args) to set the local variables to the
 ...
 * @author D.C. Ketchum
 */
public final class CommGroup {   //implements Comparator
  // static ArrayList enumName;   // this need to be populated in makeCommGroup()

  /**
   * Creates a new instance of CommGroup
   */
  private int ID;                   // This is the database ID (should alwas be named "ID"
  private String commgroup;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeCommGroup() by filling in all
   ...
   * is down
   */
  public CommGroup(ResultSet rs) throws SQLException {
    makeCommGroup(rs.getInt("ID"), rs.getString("commgroup"), rs.getString("description")
    // ,rs.getDouble(longitude)
    );
  }

  // Detail Constructor, match set up with makeCommGroup(), this argument list should be
  // the same as for the result set builder as both use makeCommGroup()
  /**
   ...
   */
  public CommGroup(int inID, String loc, String desc //, double lon
  ) {
    makeCommGroup(inID, loc, desc //, lon
    );
  }
   ...
   *
   */
  private void makeCommGroup(int inID, String loc, String desc //, double lon
  ) {
    ID = inID;
    commgroup = loc;     // longitude = lon
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((CommGroup) a).getCommGroup().compareTo(((CommGroup) b).getCommGroup());
    }
    public boolean equals(Object a, Object b) {
      if( ((CommGroup)a).getCommGroup().equals( ((CommGroup) b).getCommGroup())) return true;
      return false;
    }
//  }*/
}
