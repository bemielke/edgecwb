...
/**
 * Operator.java If the name of the class does not match the name of the field in the class you may
 * need to modify the rs.getSTring in the constructor.
 *
 * This Operator templace for creating a DBConnectionTh database object. It is not really needed by
 * the OperatorPanel which uses the DBObject system for changing a record in the database file.
 * However, you have to have this shell to create objects containing a DBConnectionTh record for
 ...
 * <br>
 * The Operator should be replaced with capitalized version (class name) for the file. operator
 * should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeOperator(list of data args) to set the local variables to the
 ...
 * @author D.C. Ketchum
 */
public final class Operator {    //implements Comparator
  // static ArrayList enumName;   // this need to be populated in makeOperator()

  /**
   * Creates a new instance of Operator
   */
  private int ID;                   // This is the DBConnectionTh ID (should alwas be named "ID"
  private String operator;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeOperator() by filling in all
   ...
   * is down
   */
  public Operator(ResultSet rs) throws SQLException {
    makeOperator(rs.getInt("ID"), rs.getString("operator"), rs.getString("description"),
            rs.getString("text")
    // ,rs.getDouble(longitude)
    );
  }

  // Detail Constructor, match set up with makeOperator(), this argument list should be
  // the same as for the result set builder as both use makeOperator()
  /**
   ...
   * @param txt text about the operator.
   */
  public Operator(int inID, String loc, String desc, String txt //, double lon
  ) {
    makeOperator(inID, loc, desc, txt //, lon
    );
  }
   ...
   *
   */
  private void makeOperator(int inID, String loc, String desc, String txt //, double lon
  ) {
    ID = inID;
    operator = loc;     // longitude = lon
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((Operator) a).getOperator().compareTo(((Operator) b).getOperator());
    }
    public boolean equals(Object a, Object b) {
      if( ((Operator)a).getOperator().equals( ((Operator) b).getOperator())) return true;
      return false;
    }
//  }*/
}
