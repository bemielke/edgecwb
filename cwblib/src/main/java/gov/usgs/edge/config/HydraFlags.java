 ...
 *
 * This Flags templace for creating a database database object. It is not really needed by the
 ...
 * @author D.C. Ketchum
 */
public final class HydraFlags {    //implements Comparator
   ...
   * is down
   */
  public HydraFlags(ResultSet rs) throws SQLException {
    makeFlags(rs.getInt("ID"), rs.getString("hydraflags"), rs.getString("description")
   ...
   * @param rs The ResultSet already SELECTED
   */
  public HydraFlags(DBTable rs) {
    makeFlags(rs.getInt("ID"), rs.getString("hydraflags"), rs.getString("description")
    // ,rs.getDouble(longitude)
    );
  }

  public void reload(DBTable rs) {
    updateData(rs);
  }
   ...
   */
  public HydraFlags(int inID, String loc, String desc //, double lon
  ) {
    makeFlags(inID, loc, desc //, lon
    );
  }
   ...
   *
   */
  private void makeFlags(int inID, String loc, String desc //, double lon
  ) {
    ID = inID;
