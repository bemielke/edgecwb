...
/**
 * SNWEventid.java If the name of the class does not match the name of the field in the class you
 * may need to modify the rs.getSTring in the constructor.
 *
 * This SNWEventid template for creating a MySQL database object. It is not really needed by the
 * SNWEventidPanel which uses the DBObject system for changing a record in the database file.
 ...
 * <br>
 * The SNWEventid should be replaced with capitalized version (class name) for the file. snweventid
 * should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeSNWEventid(list of data args) to set the local variables to
 ...
 * @author D.C. Ketchum
 */
public final class SNWEventid //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeSNWEventid()

  /**
   * Creates a new instance of SNWEventid
   */
  private int ID;                   // This is the MySQL ID (should alwas be named "ID"
  private String snweventid;             // This is the main key which should have same name
  ...
  // Put in correct detail constructor here.  Use makeSNWEventid() by filling in all
   ...
   * is down
   */
  public SNWEventid(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeSNWEventid(rs.getInt("ID"),
            rs.getString("snweventid"),
            rs.getString("description")
    // ,rs.getDouble(longitude)
    );
  }

  // Detail Constructor, match set up with makeSNWEventid(), this argument list should be
  // the same as for the result set builder as both use makeSNWEventid()
  /**
   ...
   * @param description
   */
  public SNWEventid(int inID, String loc, String description //USER: add fields, double lon
  ) {
    makeSNWEventid(inID, loc, "" //, lon
    );
  }
   ...
   *
   */
  private void makeSNWEventid(int inID, String loc, String desc //USER: add fields, double lon
  ) {
    ID = inID;
    snweventid = loc;     // longitude = lon
   ...
   */
//  public String getSNWEventid() {return snweventid.toString();}
  public long getMask() {
    return 1L << (ID - 1);
  }
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((SNWEventid) a).getSNWEventid().compareTo(((SNWEventid) b).getSNWEventid());
    }
    public boolean equals(Object a, Object b) {
      if( ((SNWEventid)a).getSNWEventid().equals( ((SNWEventid) b).getSNWEventid())) return true;
      return false;
    }
//  }*/
}
