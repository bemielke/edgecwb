 ...
 *
 * This OwnerAllow templace for creating a MySQL database object.  It is not
 * really needed by the OwnerAllowPanel which uses the DBObject system for
 ...
 *     of data.  
 *<br>
 * The OwnerAllow should be replaced with capitalized version (class name) for the
 * file.  adallow should be replaced with the lower case version (for variables).
 *
 * Both the constructors use the "makeOwnerAllow(list of data args) to set the
 ...
 * @author  D.C. Ketchum
 */
public class OwnerAllow     //implements Comparator
{   // static ArrayList enumName;   // this need to be populated in makeOwnerAllow()
  
  /** Creates a new instance of OwnerAllow */
  private int ID;                   // This is the MySQL ID (should alwas be named "ID"
  private String adallow;             // This is the main key which should have same name
                            // as the table which it is a key to.  That is
                            // Table "category" should have key "category"
  
  // USER: All fields of file go here
  //  double longitude
  private int ownerID, userID;
  
  // Put in correct detail constructor here.  Use makeOwnerAllow() by filling in all
   ...
   */
  public OwnerAllow(ResultSet rs) throws SQLException {  //USER: add all data field here
    makeOwnerAllow(rs.getInt("ID"), rs.getString("adallow")
        , rs.getInt("ownerid"), rs.getInt("userid")                    // ,rs.getDouble(longitude)
    );
  }
  
  // Detail Constructor, match set up with makeOwnerAllow(), this argument list should be
  // the same as for the result set builder as both use makeOwnerAllow()
   ...
   */
  public OwnerAllow(int inID, String loc, int ownID, int usID   //USER: add fields, double lon
    ) {
    makeOwnerAllow(inID, loc, ownID, usID     //, lon
    );
  }
   ...
   **/
  private void makeOwnerAllow(int inID, String loc, int ownID, int usID    //USER: add fields, double lon
  ) {
    ID = inID;  adallow=loc;     // longitude = lon
    ...
    ownerID=ownID; userID=usID;
    
  }
  /** return the key name as the string
   *@return the key name */
  public String toString2() { return adallow;}
  /** return the key name as the string
   *@return the key name */
  @Override
  public String toString() { return adallow;}
  // getter
   ...
   */
  public String getOwnerAllow() {return adallow;}
  ...
  /*  public class EquipCompare implements Comparator {
    public int compare(Object a, Object b) {
      return ((OwnerAllow) a).getOwnerAllow().compareTo(((OwnerAllow) b).getOwnerAllow());
    }
    public boolean equals(Object a, Object b) {
      if( ((OwnerAllow)a).getOwnerAllow().equals( ((OwnerAllow) b).getOwnerAllow())) return true;
      return false;
    }
//  }*/
