...
public class Steim1 {
  static int end;
  public static int getRIC() {return end;}
	/**
    ...
    decode(b, numSamples,samples,swapBytes, bias);
    return samples;
  }
/**
	 *  Decode the indicated number of samples from the provided byte array and
	 ...
	 *  @param numSamples the number of samples that can be decoded from array
	 *  <b>b</b>
 *    @param samples User array to put samples, must be at least numSamples long
	 *  @param swapBytes if true, swap reverse the endian-ness of the elements of
	 *  byte array <b>b</b>.
	 *  @param bias the first difference value will be computed from this value.
	 *  If set to 0, the method will attempt to use the X(0) constant instead.
	 ...
	 *  bytes.
	 */
	public static void decode(byte[] b, int numSamples, int [] samples, boolean swapBytes, int bias) throws SteimException {
		...
		if(samples.length < numSamples) throw new SteimException("Input sample buffer is too small "+samples.length+" need "+numSamples);
		int[] tempSamples;
	 ...
	 *
   * @param numSamples to return
   * @param swapBytes if true, swap the bytes
   * @return uncompressed data samples.
   * @throws gov.usgs.anss.seed.SteimException
   * @param b the compressed data bytes
	 */
	...
	* @param bias offset for use as a constant for the first difference, otherwise set to 0
	* @return SteimFrameBlock containing encoded byte array, or cannot encode more than 63 frames
   * @throws gov.usgs.anss.seed.SteimException
	*/
	public static SteimFrameBlock encode(int[] samples, int frames, int bias) throws SteimException {
		...
		int diffCount;  // how many sample diffs we put into current word
		int maxSize;    // the maximum diff value size encountered
		int curSize;    // size of diff value currently looked at
   ...
   * @param samples samples to be compressed
   * @param frames The number of frames in a full block
   * @return StimeFrameBlock with encoded data
   * @throws gov.usgs.anss.seed.SteimException
	 */
