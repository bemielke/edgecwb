...
/**
 * This class handle queries from VMS and is now obsolete
 *
 * @author davidketchum
 */
public final class VMSServer extends Thread {
  ...
  private final class ShutdownVMSServer extends Thread {
  ...
  private final class VMSServerHandler extends Thread {
    ...
    public void run() {
      ...
      for (String args21 : args2) {
        // put quoted string back together
        if (args21.trim().equals("")) {
          if (inquote) {
            args[j] += " ";
          }
          continue;
        ...
        if (inquote) {
          if (args21.substring(args21.length() - 1).equals("'") || args21.substring(args21.length() - 1).equals("\"")) {
            args[j++] += " " + args21.substring(0, args21.length() - 1);
            inquote = false;
          } else {
            args[j] += " " + args21;
          }
        } else {
          if (args21.substring(0, 1).equals("'") || args21.substring(0, 1).equals("\"")) {
            args[j] = args21.substring(1); // remove the quote
            inquote = true;
          } else {
            args[j++] = args21; // No quote, nor one found, just add it on
          }
        }
      }
      System.arraycopy(args, 0, args2, 0, j);
      args = new String[j];
      System.arraycopy(args2, 0, args, 0, j);
          ...
          if (blks.size() > 0) {
            ...
            for (int i = 0; i <= span.getNsamp(); i++) {
              if (i == span.getNsamp() || samples[i] == NODATA) {
                out.write(blks.get(0).getSeedNameString().getBytes(), 0, 12);
                ...
                Util.prta("VSH: " + blks.get(0).getSeedNameString() + " " + now.get(Calendar.YEAR) + " "
