...
/**
 * This class generates 4096 byte mini-seed from the returned blocks from a query. It uses the
 ...
 *
 * 3) The list of blocks are then processed into 4096 byte blocks by adding the data sections of
 * full data frames together. The status sections (ClockIO bits, data quality bits, and activities
 * bits are "OR"ed together from the assembly of blocks. The clock quality in Blockette 1001 is the
 * minimum clock quality from the assembled blocks.
 *
 * 4) The blocks are output when a) There is not enough room in the 4096 to add the next one, b) The
 * block to be added would cause the reverse integration error for the assembled block c) The last
 * block added is a "partial" that is not a full set of data frames. d) The next block to add is not
 * contiguous in time
 *
...
public class DCCOutputer extends Outputer implements MiniSeedOutputHandler {
  ...
  public void makeFile(String comp, String filename, String filemask, ArrayList<MiniSeed> blks,
    ...
    if (blks.get(0).getSeedNameSB().substring(7, 10).equals("ACE")) {
      return;
    }
    ...
    for (int i = blks.size() - 1; i >= 0; i--) {
      if (blks.get(i).getRate() < 1.001) {
        if (blks.get(i).getSeedNameSB().substring(7, 9).equals("BH")) {
      ...
      try {
        ...
        Util.prt("Got array OOB copy exception e=" + e);
        e.printStackTrace();
      }
      if (ms2.getEncoding() == 11) {
    ...
    for (MiniSeed blk : blks) {
      ...
      if (ms2.getBlockette1001() != null) {
        if (ms2.getTimingQuality() < 0 || ms2.getTimingQuality() > 100) {
          ...
          Util.prt("AFter delete 1000 worked=" + worked + " " + ms2);
        }
      }
      if (ms2.getRate() > rate) {
        rate = ms2.getRate();
    ...
    // Set up the
    frameCount = 0;                     // This must be zero for writeBlock() to initilize and not write
    ...
    for (MiniSeed list1 : list) {
      long start = list1.getGregorianCalendar().getTimeInMillis();
      ...
      if (Math.abs(start - expected) < gapThreshold) {
        ...
        try {
          ...
          RawToMiniSeed rtms = new RawToMiniSeed(ms.getSeedNameSB(), rate, 63,
          ...
          Util.prt("   *** steim1/2 err=" + e.getMessage() + " i=" + iblk + " ms=" + list1.toString());
        }
      } else {
        writeBlock(list1);
        addBlock(list1);
    ...
    // 
    if (check) {
      ZeroFilledSpan checkOutput = new ZeroFilledSpan(outblks, 2147000000);
  ...
  /**
   * add this miniseed blockette to one being built in dummy. The activity, IOClock, and data
   * quality flags are ORed together and the blockette 1001 clock quality is set to minimum.
   *
  ...
  private void addBlock(MiniSeed ms) {
    ...
    do {
      int maxns = (int) ((86400000L - (ms.getGregorianCalendar().getTimeInMillis() % 86400000L) + ((long) 1000 / ms.getRate() - 1))
              * ms.getRate() / 1000);
      if (maxns < ms.getNsamp()) {    // if we need to shorten this block to make it stop at the day boundardy, do so
        ...
        try {
          ...
          System.arraycopy(ms.getBuf(), ms.getDataOffset(), frames2, 0, ms.getBlockSize() - ms.getDataOffset());
          int[] samples = null;
          ...
          if (e.getTimeInMillis() < dropDeadEnd) {
            ...
            RawToMiniSeed rtms = new RawToMiniSeed(ms.getSeedNameSB(), ms.getRate(), 63,
      ...
      if (rateFactor != ms.getRateFactor()) {
        Util.prt("Changing rate factor at was " + rateFactor + " is " + ms.getRateFactor() + " ms=" + ms);
        rateFactor = ms.getRateFactor();
      }
      if (rateMult != ms.getRateMultiplier()) {
        Util.prt("Changing rate multiplier was " + rateMult + " is " + ms.getRateMultiplier() + " ms=" + ms);
  ...
  /**
   * examine the first difference, the last reverse constant (point prior to first one in this
   * miniseed record and correct the difference if it does not match up. This allows us to fix
   * differences that are due to separate compression runs that are contiguous (the 1st difference
   * in any compression one is to zero not to the correct value)
   */
  ...
  /**
   * write out the current block (add it to the list in outblks) and fill in needed header data for
   * next one, call with frameCount=0 to cause just the setup to be taken. I All of the setup
   * variables are reset after a call.
   *
   * @param ms The miniseed block which will start off the next block (the one too long to put in
   * current block)
   */
  public void writeBlock(MiniSeed ms) {
    ...
    if (frameCount > 0) {
      ...
      try {
        ...
        Util.prt("**** final check block gave bad seedname exception e=" + e.getMessage());
        badSeedname = true;
      } catch (SteimException e) {
        steimError = true;
    ...
    maxnsamp = (int) ((86400000L - (ms.getGregorianCalendar().getTimeInMillis() % 86400000L) + (int) (1000 / ms.getRate() - 1)) / 1000.
            * ms.getRate());
    frameCount = 0;
    minClockQual = 10000;
    activityFlags = 0;
