...
/**
 * This class generates 512 byte mini-seed from the returned blocks from a query. It uses the
 ...
 * blocks at these boundaries.
 *
 * @author davidketchum
 */
public final class DCC512Outputer extends Outputer implements MiniSeedOutputHandler {
  ...
  //private final byte [] empty1000 = {3,(byte) 232,0,56,11,1,12,0};
  //private final byte [] empty1001 = {3,(byte) 233,0,0,0,0,0,7};

  int maxnsamp;                 // storage for the most samples that could be in a block before midnight
  long dropDeadEnd;
  boolean nooutput;               // if true, no files are created, but output blocks are present

  public ArrayList<MiniSeed> getOutblocks() {
    return outblks;
  }

  @Override
  public void makeFile(String comp, String filename, String filemask, ArrayList<MiniSeed> blksin,
          java.util.Date beg, double duration, String[] args) throws IOException {
    //MiniSeed ms2=null;
    ArrayList<MiniSeed> blks = new ArrayList<>(blksin.size());
    for (MiniSeed blksin1 : blksin) {
      blks.add(blksin1);
    }
    frames = new byte[4096 - 64];   // scratch space for decompression
    runs = new ArrayList<>(100);
    dummy = new byte[512];
    ...
    if (blks.get(0).getSeedNameString().substring(7, 10).equals("ACE")) {
      return;
    }
    ...
    for (int i = blks.size() - 1; i >= 0; i--) {
      if (blks.get(i).getRate() < 1.001 && blks.get(i).getRate() > 0.0001) {
        if (blks.get(i).getSeedNameString().substring(7, 9).equals("BH")) {
      ...
      MiniSeed ms2 = blks.get(i);
      if (ms2 == null) {
        Util.prt(" *** Got unexpected null miniseed block in i=" + i);
      } else {
        try {
          int reverse = 0;
          int[] samples = null;
          ...
          Util.prt("Got array index out-of-bounds on " + comp + " beg=" + beg + " dur=" + duration
                  + " file=" + filename + " nd=" + Util.getNode());
          SendEvent.debugEvent("DCC512OOR", "Query OOB error " + Util.getNode(), this);
          Util.prta(Util.getNode() + "/" + Util.getAccount() + " "
                  + Util.ascdate() + " " + Util.asctime() + " Got array index out-of-bounds on "
                  + comp + " beg=" + beg + " dur=" + duration + " file=" + filename + " nd=" + Util.getNode() + " blk=" + i + "\n"
                  + "offset=" + ms2.getDataOffset() + " blksize=" + ms2.getBlockSize()
                  + " frames.size=" + frames.length + " bufsize=" + ms2.getBuf().length + "\n");
          blks.remove(i);
          continue;
        }
        ...
        if (ms2.getBlockSize() > 512) {
          try {
            ...
            blks.addAll(Arrays.asList(blk512));
    ...
    boolean q330 = false;
    for (String Q330S1 : Q330S) {
      if (comp.substring(2, 7).trim().equals(Q330S1)) {
        q330 = true;
        break;
      }
    }
    ...
    for (String arg : args) {
      ...
      if (arg.equals("-dccdbg")) {
        dbg = true;
      }
      if (arg.equals("-nooutput")) {
        nooutput = true;
        check = true;
      } // this only makes sense if trying to get output blocks
    }
    ...
    for (MiniSeed blk : blks) {
      MiniSeed ms2 = (MiniSeed) blk;
      // The GSN would prefer no blockette 1001 if it contains invalid data
    ...
    Run r = runs.get(0);
    int currRun = 0;
    ArrayList<MiniSeed> list = new ArrayList<>(1000); // this will contain the purged list of blocks
    ...
    while (!done) {
      ...
      if (dbg) {
        Util.prt("This run ends with " + r.getMS(r.getNBlocks() - 1) + " end tim="
                + r.getMS(r.getNBlocks() - 1).getEndTimeString());
      }
      ...
      if (ilatest >= 0) {
        ...
        if (iblk != runs.get(ilatest).getNBlocks()
                ...
                && runs.get(ilatest).getMS(Math.max(iblk - 1, 0)).getGregorianCalendarEndTime().compareTo(r.getEnd()) > 0) {
          iblk = iblk - 1;
          if (dbg) {
            Util.prt("ms = " + Util.asctime(runs.get(ilatest).getMS(iblk).getGregorianCalendar())
                    + " r.end=" + Util.asctime(r.getEnd())
                    + " compareTO=" + runs.get(ilatest).getMS(iblk).getGregorianCalendar().compareTo(r.getEnd())
                    + " ms=" + runs.get(ilatest).getMS(iblk).toString());
          }
          if (dbg) {
          ...
          if (dbg) {
            Util.prt("Last one is still before end ms=" + Util.asctime(runs.get(ilatest).getMS(iblk - 1).getGregorianCalendar())
                    + " r.end=" + Util.asctime(r.getEnd())
                    + " compareTO=" + runs.get(ilatest).getMS(iblk - 1).getGregorianCalendar().compareTo(r.getEnd())
                    + " ms=" + runs.get(ilatest).getMS(iblk - 1).toString());
          }
        }
        if (iblk < 0) {
          iblk = 0;
    ...
    if (check) {
      ...
      outblks = new ArrayList<>(blks.size());
      if (!nooutput) {
    ...
    // Now we need to go through the frames agregating them into 4096 byte frames as needed
    if (model != null) {
    ...
    for (int i = 1; i < list.size(); i++) {
      long start = list.get(i).getTimeInMillisTruncated();
      ...
      if (Math.abs(start - expected) < gapThreshold) {
        ...
        if (ms.getBlockSize() - ms.getDataOffset() > frames.length) {
          SendEvent.debugEvent("BufOverDCC", "Buffer overrund " + Util.getNode() + "/" + Util.getAccount() + " Buf Overrun", this);
        }
        ...
        try {
          ...
          if (newnsamp > 0 && data != null) {
            ...
            try {
              ...
              Util.prt("OOB error offset=" + offset + " data.length=" + data.length + " newnsamp=" + newnsamp);
            }
            int usec = ms.getUseconds();
            sss.setTimeInMillis(ms.getTimeInMillisTruncated());    // start time of ms buffer
            sss.add(Calendar.MILLISECOND, (int) (offset / rate * 1000. + 0.5));// add offset to first sample

            RawToMiniSeed rtms = new RawToMiniSeed(ms.getSeedNameSB(), rate, 7,
                    ...
                    sss.get(Calendar.MILLISECOND) * 1000 + usec,
                    1, null);
  ...
  private void write512(MiniSeed ms) {
    try {
      ...
      if (ms.getNsamp() > 0 && ms.getRate() > 0.) {
        expected = ms.getTimeInMillisTruncated() + (long) (ms.getNsamp() * 1000. / ms.getRate() + 0.5);
      }
      if (check) {
        outblks.add(ms);
      }
  ...
  /**
   * OBSOLETE? for 512? examine the first difference, the last reverse constant (point prior to
   * first one in this miniseed record and correct the difference if it does not match up. This
   * allows us to fix differences that are due to separate compression runs that are contiguous (the
   * 1st difference in any compression one is to zero not to the correct value)
   */
  private boolean fixit(int rev, byte[] buf) {
    ...
    int diff;                     // this will contain the actual first difference
   ...
   * accepts it
   */
  private final class Run implements Comparable<Run> {
