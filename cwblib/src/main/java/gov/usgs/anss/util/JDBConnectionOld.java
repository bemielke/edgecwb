...
/**
 * This is a static holder for JDBC connections to the MySQL database.
 * Connections are accessed by keys, which are just Strings. Some methods do not
 * take a key; they act as if null were provided as the key.
 */
public final class JDBConnectionOld
{
  /**
   * This contains the default properties used by every connection. It is
   * initialized in the static initializer below.
   */
  ...
  public boolean isPostgres() {return postgres;}
  ...
  public void useOracle() {oracle=true;}
  /**
   * Create a connection with the specified host, catalog, user, and password
   * and make it the default connection. If useSSL is true, have the connection
   * use SSL.
   *
   * @param host the host name of the server to connect to
   * @param catalog the catalog to use once connected. If null, don't connect to
   ...
   */
 /* public JDBConnectionOld(String host, String catalog, String user,
          ...
          String password, boolean useSSL, String name, String vend) throws SQLException
  {
    Connection c;
    Properties props;
    if(host.contains(":")) {   // This is a URL host, break it up into the pieces
      String [] parts = host.split(":");
      if(parts.length > 0) host = parts[0];
      if(parts.length > 1) catalog = parts[1];
      if(parts.length > 1) vendor = parts[2];
      if(parts.length > 1) schema = parts[3];
    }
    if(vend == null) {vendor="mysql"; mysql=true;}
    else if(vend.equalsIgnoreCase("oracle")) {vendor="oracle"; oracle = true;}
    else if(vend.equalsIgnoreCase("postgres")) {vendor="postgres"; postgres=true;}
    else {vendor="mysql"; mysql=true;}
    try {
    ...
    if(oracle) registerDriver("oracle.jdbc.driver.OracleDriver");
    else if(postgres) registerDriver("org.postgresql.Driver");
    else registerDriver(UC.JDBCDriver());
    ...
    try {
      ...
      Util.prta("JDBConnection1 o.k. to "+host+" driver="+getDriver(name)+" db="+catalog+" schema="+schema+" user="+user+" ssl="+useSSL+" as "+name);
    } catch (URISyntaxException e) {
      Util.prta("JDBConnection1 FAILED to "+host+" driver="+getDriver(name)+" db="+catalog+" schema="+schema+" user="+user+" ssl="+useSSL+" as "+name);
      throw new SQLException(e.getMessage());
    }
    catch(SQLException e) {
      Util.prt("JDBConnection SQL error to "+host+" driver="+getDriver(name)+" db="+catalog+" schema="+schema+" user="+user+" ssl="+useSSL+" as "+name);
      throw e;
    }
  }
  ...
  /**
   * Create a connection with the specified host, catalog, user, and password
   * and make it the default connection.
   *
   * @param host the host name of the server to connect to
   * @param catalog the catalog to use once connected. If null, don't connect to
   *                any particular catalog
  ...
  /**
   * Create a connection from a driver and a URL and associate it with name,
   * closing any previous connection associated with name.
   *
   ...
   * @exception SQLException Usually network or database is down
   */
  public JDBConnectionOld(String driver, String url, String name)
    throws SQLException
  {
    Connection c;
    String host;
   ...
   * @exception SQLException usually network or database is down
   */
  public JDBConnectionOld(String driver, String url) throws SQLException
  {
  ...
  /**
   * Set the database (JDBC calls this a "catalog") to use for the connection
   ...
   *
   * @param name the key associated with the connection
   * @return the database connection, or null if none has been associated
   *         with name
   */
  public static Connection getConnection(String name)
  { if(name == null) return (Connection) connections.get(null);
    if(name.equals("")) return (Connection) connections.get(null);
  ...
  /**
   * Get the current database (JDBC calls this a "catalog") in use by the
  ...
  public static void setConnection(Connection C) { connections.put(null, C);}
  /**
  ...
  /**
   * Extract the host name from a JDBC URL. This may be specific to MySQL
   * Connector/J URLs.
   *
    ...
    for(int i=0; i<tokens.length; i++) {
      if(i % 2 == 1) {
        if(tokens[i].contains("&")) {    // Its a name, look it up
          ...
          String s = "SELECT * FROM anss_poc_people WHERE last_name ='"+name[0].trim()+"'";
          if(name.length > 1) s += " AND first_name LIKE'"+name[1].trim()+"%'";
          try {
            try (ResultSet rs = oracleght.executeQuery(s)) {
              if(rs.next()) {
                if(name.length == 2) {
                  sb.append(" ").append(rs.getString("first_name")).append(" ").append(rs.getString("last_name")).
                          append(" <").append(rs.getString("emp_email")).append("> Office Ph:").
          ...
          try {
            String s = "SELECT * FROM anss_poc_orgs WHERE name LIKE '%"+tokens[i]+"%'";
            ResultSet rs = oracleght.executeQuery(s);
            if(rs.next()) {
              int org_id=rs.getInt("org_id");
              rs.close();
              rs = oracleght.executeQuery("SELECT * from anss_poc_people WHERE org_id="+org_id);
              while(rs.next()) {
                refs.append(rs.getString("first_name")).append(" ").append(rs.getString("last_name")).
                        append(" ").append(rs.getString("emp_email")).append(" ").
                        append(rs.getString("emp_off_ph")).append(" ").append(rs.getString("title")).append("\n");
              }
            }
          }
          catch(SQLException e) {
  ...
  public static void main(String args[]) {
    ...
    try{
      JDBConnectionOld mysql = new JDBConnectionOld("gacqdb","edge","ro","readonly",true, "edge","mysql");
      Statement stmtmysql = JDBConnectionOld.getConnection("edge").createStatement();
        // Get user from the Inventory database user file
      jcjbl = new JDBConnectionOld("igskcicgasordb2.cr.usgs.gov","PROD1","adm_sel","ham11*ret", true,"poc", "oracle");
      C = JDBConnectionOld.getConnection();
      C2 = JDBConnectionOld.getConnection("poc");
      Statement stmt = C.createStatement();
      // Get a new connection to the ANSS database
      UC.setConnection(JDBConnectionOld.getConnection("poc"));
      ResultSet rs = stmt.executeQuery("SELECT * FROM anss_poc_people ORDER BY FULL_NAME");
      while(rs.next()) {
